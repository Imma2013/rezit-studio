...
exports.HARMONY_SCHEMES = void 0;
...
function rot(hsl, deg) {
    return (0, convert_1.hslToRgb)({ ...hsl, h: ((hsl.h + deg) % 360 + 360) % 360 });
}
function withL(hsl, l) {
    return (0, convert_1.hslToRgb)({ ...hsl, l: Math.max(0, Math.min(1, l)) });
}
/** Colors harmonious with `base` for the given scheme, base first. */
function colorHarmony(base, scheme) {
    const hsl = (0, convert_1.rgbToHsl)(base);
    switch (scheme) {
        case "complementary":
