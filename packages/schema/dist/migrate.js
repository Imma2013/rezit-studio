...
exports.MigrationError = exports.migrations = void 0;
...
function migrateTextV1toV2(node) {
    const oldRuns = Array.isArray(node.content) ? node.content : [];
...
function mapNodesV3(nodes) {
    return nodes.map((node) => {
        ...
        if (Array.isArray(n.children))
            n = { ...n, children: mapNodesV3(n.children) };
        return n;
    });
}
...
function cmykToSrgb(c, m, y, k, a = 1) {
    return { r: (1 - c) * (1 - k), g: (1 - m) * (1 - k), b: (1 - y) * (1 - k), a };
}
function srgbToCmyk(s) {
...
function convertColor(c) {
    ...
    if (c.space === "srgb" || c.space === "p3")
        return { srgb: { r: c.r, g: c.g, b: c.b, a: c.a } };
    if (c.space === "cmyk") {
    ...
    switch (f.type) {
        ...
        case "mesh":
            return {
                ...
                gradient: "mesh",
                stops: [],
                mesh: { rows: f.rows, cols: f.cols, points: (f.points ?? []).map((p) => ({ x: p.x, y: p.y, color: convertColor(p.color) })) },
            };
        case "pattern":
            return compact({ type: "pattern", assetId: f.assetId, scale: f.scale, rotation: f.rotation, repeat: f.repeat === "none" ? "no-repeat" : f.repeat });
        case "image":
            if (f.source)
                return f; // already a v4 ImageFill
        ...
        case "gradient": // already v4; convert nested colors idempotently
            return compact({ ...f, stops: convStops(f.stops), mesh: f.mesh ? { ...f.mesh, points: f.mesh.points.map((p) => ({ ...p, color: convertColor(p.color) })) } : undefined });
        default:
            return f;
    }
}
function isColorObj(v) {
    if (typeof v.space === "string" && COLOR_SPACES.has(v.space))
        return true;
    return typeof v.srgb === "object" && v.srgb !== null && "r" in v.srgb && !("type" in v);
}
function isFillObj(v) {
    return typeof v.type === "string" && FILL_TYPES.has(v.type) && !("id" in v) && !("transform" in v);
}
function deepConvertColors(value) {
    if (Array.isArray(value))
        return value.map(deepConvertColors);
...
function migrateAnimationsV5toV6(node) {
    ...
    if (!legacy || typeof legacy !== "object")
        return node;
    const preset = LEGACY_ENTRANCE[legacy.preset];
    if (!preset)
        return node;
    return {
        ...node,
...
function mapNodesV6(nodes) {
    return nodes.map((node) => {
        let n = migrateAnimationsV5toV6(node);
        if (Array.isArray(n.children))
            n = { ...n, children: mapNodesV6(n.children) };
        if (n.child && typeof n.child === "object")
            n = { ...n, child: mapNodesV6([n.child])[0] };
        return n;
    });
}
// v13 -> v14: effect normalization (see the step comment below).
function mapNodesV14(nodes) {
    return nodes.map((node) => {
        let n = node;
        if (Array.isArray(n.effects) && n.effects.some((e) => e.kind === "shadow" && e.type === undefined)) {
            n = { ...n, effects: n.effects.map((e) => (e.kind === "shadow" && e.type === undefined ? { ...e, type: "drop" } : e)) };
        }
        if (Array.isArray(n.textEffects) && n.textEffects.some((e) => e.kind === "shadow" && e.opacity !== 1)) {
            n = { ...n, textEffects: n.textEffects.map((e) => (e.kind === "shadow" && e.opacity !== 1 ? { ...e, opacity: 1 } : e)) };
        }
        if (Array.isArray(n.children))
            n = { ...n, children: mapNodesV14(n.children) };
        if (n.child && typeof n.child === "object")
            n = { ...n, child: mapNodesV14([n.child])[0] };
        return n;
    });
}
/**
...
exports.migrations = {
    ...
    1: (file) => ({
        ...file,
        schemaVersion: 2,
        pages: (file.pages ?? []).map((p) => ({ ...p, children: mapNodesV2(p.children ?? []) })),
    }),
    // v2 -> v3: image model.
    2: (file) => ({
        ...file,
        schemaVersion: 3,
        pages: (file.pages ?? []).map((p) => ({ ...p, children: mapNodesV3(p.children ?? []) })),
    }),
    // v3 -> v4: unified color/fill model (deep color+fill conversion across the file).
    3: (file) => ({ ...deepConvertColors(file), schemaVersion: 4 }),
    ...
    5: (file) => ({
        ...file,
        schemaVersion: 6,
        pages: (file.pages ?? []).map((p) => ({ ...p, children: mapNodesV6(p.children ?? []) })),
    }),
    ...
    13: (file) => ({
        ...file,
        schemaVersion: 14,
        pages: (file.pages ?? []).map((p) => ({ ...p, children: mapNodesV14(p.children ?? []) })),
    }),
    ...
    17: (file) => {
        const legacy = file.meta?.language;
 ...
 */
function migrate(file, toVersion = schema_1.CURRENT_SCHEMA_VERSION) {
    const from = file.schemaVersion;
    if (from > toVersion) {
        throw new MigrationError(`cannot downgrade a v${from} file to v${toVersion}; migration is forward-only`, from, toVersion);
    }
    if (from === toVersion) {
    ...
    for (let v = from; v < toVersion; v++) {
        const step = exports.migrations[v];
        if (!step) {
            throw new MigrationError(`no migration registered from v${v} to v${v + 1}`, from, toVersion);
        }
...
function needsMigration(file, toVersion = schema_1.CURRENT_SCHEMA_VERSION) {
