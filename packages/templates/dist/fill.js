...
exports.validateFillRow = validateFillRow;
 ...
 */
function fillImageNode(node, imageUrl) {
    const rec = node;
    const nextSource = { assetId: imageUrl, naturalWidth: 0, naturalHeight: 0 };
    if (node.type === "image") {
 ...
 */
function applyFill(file, values) {
    ...
    for (const [nodeId, value] of Object.entries(values)) {
        ...
        if (!hasText && !hasImage)
            continue; // no value: leave default
        ...
        let applied = false;
        if (hasText)
            applied = fillTextNode(node, value.text) || applied;
        if (hasImage)
            applied = fillImageNode(node, value.imageUrl) || applied;
        if (applied)
            filled.push(nodeId);
        else
            missing.push(nodeId);
 ...
 */
function validateFillRow(fields, values) {
    for (const field of fields) {
        const v = values[field.nodeId];
        const c = field.constraints;
        const present = !!v && ((typeof v.text === "string" && v.text.length > 0) || (typeof v.imageUrl === "string" && v.imageUrl.length > 0));
