...
exports.filtersToTemplateQuery = filtersToTemplateQuery;
...
function colorMatches(t, hex, maxDist = 70) {
...
function filtersToTemplateQuery(params, workspaceId) {
    const q = {};
    if (params.q)
        q.text = params.q;
    if (params.color)
    ...
    if (params.category)
        q.category = params.category.split(",");
    if (params.scope === "personal" || params.scope === "team" || params.scope === "public" || params.scope === "all")
        q.scope = params.scope;
    if (params.format) {
        const [w, h] = params.format.split("x").map(Number);
        if (Number.isFinite(w) && Number.isFinite(h))
            q.format = { width: w, height: h };
    }
...
function textRelevance(t, q) {
    const n = q.toLowerCase();
    let s = 0;
    if (t.title.toLowerCase() === n)
        s += 100;
    else if (t.title.toLowerCase().includes(n))
        s += 50;
    for (const tag of t.tags)
        if (tag.toLowerCase().includes(n))
            s += 8;
    for (const cat of t.categories)
        if (cat.toLowerCase().includes(n))
            s += 5;
    return s;
}
function templateMatches(t, query) {
    if (!scopeAllows(t, query))
        return false;
    if (query.text && textRelevance(t, query.text) === 0)
        return false;
    if (query.color && !colorMatches(t, query.color))
        return false;
    const styles = asArray(query.style);
    if (styles.length && !styles.some((s) => (t.style.styleTags ?? []).includes(s)))
        return false;
    const cats = asArray(query.category);
    if (cats.length && !cats.some((c) => t.categories.includes(c)))
        return false;
    if (query.format && (t.format.width !== query.format.width || t.format.height !== query.format.height))
        return false;
    return true;
}
