...
exports.parseInlineMarkdown = parseInline;
exports.paragraphFromText = paragraphFromText;
const model_1 = require("./model");
// ---------------------------------------------------------------------------
// Inline run rendering
// ---------------------------------------------------------------------------
function hasMark(marks, m) {
    if (!marks)
        return false;
...
function runToMarkdown(run) {
    ...
    if (hasMark(run.marks, "code"))
        text = "`" + text + "`";
    if (hasMark(run.marks, "strike"))
        text = "~~" + text + "~~";
    if (hasMark(run.marks, "italic"))
        text = "_" + text + "_";
    if (hasMark(run.marks, "bold"))
        text = "**" + text + "**";
    if (hasMark(run.marks, "underline"))
        text = "<u>" + text + "</u>";
    if (run.link)
        text = "[" + text + "](" + run.link + ")";
    return text;
}
function richToMarkdown(rt) {
    return rt.runs.map(runToMarkdown).join("");
}
// ---------------------------------------------------------------------------
// Block -> markdown
// ---------------------------------------------------------------------------
function listItemToMarkdown(item, style, index) {
...
function tableToMarkdown(block) {
    ...
    const cellText = (row, c) => row.cells[c] ? richToMarkdown(row.cells[c]) : "";
...
function parseInline(s) {
    ...
    const matchDelim = (open, close, mark) => {
    ...
    while (i < s.length) {
        ...
        if (matchDelim("**", "**", "bold"))
            continue;
        if (matchDelim("~~", "~~", "strike"))
            continue;
        if (matchDelim("`", "`", "code"))
            continue;
        if (matchDelim("_", "_", "italic"))
            continue;
        buf += s[i];
        i++;
    }
...
function parseTable(lines, start) {
    ...
    if (sep === undefined || !TABLE_SEP_RE.test(sep) || !sep.includes("|"))
        return null;
    const headerCells = splitTableCells(header);
    const aligns = splitTableCells(sep).map((s) => {
...
function markdownToBlocks(md) {
    ...
    const flushList = () => {
        if (listAccum) {
            blocks.push((0, model_1.newList)(listAccum.style, listAccum.items));
            listAccum = null;
        }
    };
    while (i < lines.length) {
        ...
        if (fence) {
            ...
            i++; // closing fence
            blocks.push((0, model_1.newCode)(body.join("\n"), lang || undefined));
            continue;
        }
        ...
        if (heading) {
            ...
            blocks.push((0, model_1.newHeading)(level, parseInline(heading[2])));
            i++;
            continue;
        }
        // Image (own paragraph).
        ...
        if (image) {
            flushList();
            blocks.push((0, model_1.newImage)({ url: image[2], alt: image[1] || undefined }));
            i++;
            continue;
        }
        ...
        if (quote) {
            flushList();
            blocks.push((0, model_1.newQuote)(parseInline(quote[1])));
            i++;
            continue;
        }
        // Checklist item.
        ...
        if (check) {
            ...
            const item = (0, model_1.newListItem)(parseInline(check[3]), depth);
        ...
        if (bullet) {
            ...
            const item = (0, model_1.newListItem)(parseInline(bullet[2]), depth);
        ...
        if (numbered) {
            ...
            const item = (0, model_1.newListItem)(parseInline(numbered[2]), depth);
        ...
        flushList();
        blocks.push((0, model_1.newParagraph)(parseInline(line)));
        i++;
    }
    flushList();
    return blocks;
...
function paragraphFromText(text) {
    return (0, model_1.newParagraph)((0, model_1.plainToRichText)(text));
}
