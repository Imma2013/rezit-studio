 ...
 * mutation.
 */
export function findNodeMap(ydoc, nodeId) {
...
function searchChildren(children, nodeId) {
 ...
 */
export function snapshotNodes(ydoc, ids) {
    const snap = new Map();
    for (const id of ids) {
        const map = findNodeMap(ydoc, id);
        snap.set(id, map ? yToJson(map) : null);
    }
    return snap;
}
/**
 ...
 * edit (no echo loop).
 */
export function restoreNodes(ydoc, snapshot) {
    const corrected = [];
    ydoc.transact(() => {
        for (const [id, prior] of snapshot) {
            ...
            if (prior === null) {
                ...
                if (current && removeNode(ydoc, id))
                    corrected.push(id);
                continue;
            }
            if (current) {
                // Present in both: revert any change with minimal ops.
                const now = yToJson(current);
 ...
 *  found and removed. */
function removeNode(ydoc, nodeId) {
    const root = ydoc.getMap(DESIGN_ROOT_KEY);
    const pages = root.get("pages");
    if (!(pages instanceof Y.Array))
        return false;
...
function removeFromChildren(children, nodeId) {
 ...
 */
function reinsertNode(ydoc, nodeId, prior) {
    const root = ydoc.getMap(DESIGN_ROOT_KEY);
    const pages = root.get("pages");
    if (!(pages instanceof Y.Array) || pages.length === 0)
        return false;
