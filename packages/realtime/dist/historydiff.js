...
import { walkNodes } from "@hc/schema";
function contentSig(n) {
    const rest = { ...n };
    delete rest.id;
 ...
 */
export function diffLabel(before, after) {
    ...
    for (const [id, nb] of b) {
        ...
        if (na.w !== nb.w || na.h !== nb.h || na.sx !== nb.sx || na.sy !== nb.sy)
            resized++;
        else if (na.x !== nb.x || na.y !== nb.y)
            moved++;
        else if (na.rot !== nb.rot)
            rotated++;
        else if (na.sig !== nb.sig)
            edited++;
    }
    ...
    if (nonzero.length === 1)
        return `${nonzero[0][1]} ${plural(nonzero[0][0], "element")}`;
    if (nonzero.length > 1) {
    ...
    const pa = before.pages ?? [];
    const pb = after.pages ?? [];
    if (pb.length > pa.length)
        return `Added ${plural(pb.length - pa.length, "page")}`;
    if (pa.length > pb.length)
        return `Removed ${plural(pa.length - pb.length, "page")}`;
    ...
    for (const p of pb) {
        ...
        if ((p.name ?? "") !== (q.name ?? ""))
            renamed++;
        else if (p.width !== q.width || p.height !== q.height)
            pageResized++;
        else if (JSON.stringify(p.background ?? null) !== JSON.stringify(q.background ?? null))
            bgChanged++;
    }
    if (renamed)
        return `Renamed ${plural(renamed, "page")}`;
    if (pageResized)
        return `Resized ${plural(pageResized, "page")}`;
    if (bgChanged)
        return bgChanged === 1 ? "Changed page background" : "Changed page backgrounds";
    return "";
}
