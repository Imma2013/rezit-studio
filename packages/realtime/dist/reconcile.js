...
function isKeyedArray(arr) {
    return (arr.length > 0 &&
        arr.every((it) => isPlainObject(it) && typeof it.id === "string"));
}
/**
 ...
 */
function isParagraphArray(arr) {
    return (arr.length > 0 &&
        arr.every((it) => isPlainObject(it) &&
            Array.isArray(it.runs) &&
            isPlainObject(it.style) &&
            it.id === undefined));
}
 ...
 */
function isCanonicalRuns(runs) {
    for (let i = 0; i < runs.length; i++) {
        const r = runs[i];
        if (!isPlainObject(r) || typeof r.text !== "string" || r.text.length === 0)
            return false;
        if (i > 0 && deepEqual(runPayload(runs[i - 1]), runPayload(r)))
            return false;
    }
    return true;
}
...
export function yToJson(value) {
    ...
    if (value instanceof Y.Text) {
        return deltaToRuns(value.toDelta());
    }
    if (value instanceof Y.Array) {
        ...
        if (items.length > 0 && items.every((it) => isPlainObject(it) && typeof it.id === "string")) {
            const objs = items;
            ...
            if (ranks.some((r) => r !== null)) {
                const order = objs.map((obj, i) => ({ obj, rank: ranks[i] ?? Number.POSITIVE_INFINITY, id: obj.id }));
    ...
    if (value instanceof Y.Map) {
        ...
        for (const [k, v] of value.entries()) {
            if (k === ORDER_KEY || k === RUNS_STASH_KEY)
                continue;
            obj[k] = yToJson(v);
        }
        ...
        if (typeof stash === "string" && Array.isArray(obj.runs) && obj.runs.length === 0) {
            try {
                ...
                if (Array.isArray(parsed) && parsed.every((r) => isPlainObject(r) && typeof r.text === "string")) {
                    obj.runs = parsed;
                }
            }
            catch {
 ...
 */
function reconcileMap(target, source) {
...
function reconcileChild(parent, key, sv) {
...
function reconcileArray(target, source) {
 ...
 */
function reconcilePlainArray(target, source) {
    if (deepEqual(yToJson(target), source))
        return;
    if (target.length > 0)
 ...
 */
function reconcileKeyedArray(target, source) {
...
function assignOrder(items) {
    ...
    while (i < n) {
        ...
        while (j < n && !keep[j])
            j++;
        const left = i > 0 ? keys[i - 1] : null; // i-1 is an anchor (or start)
        const right = j < n ? keys[j] : null; // j is an anchor (or end)
 ...
 */
function spreadRanks(left, right, count) {
    const out = [];
    if (left !== null && right !== null) {
        if (right - left <= ORDER_MIN_GAP)
            return null;
        const step = (right - left) / (count + 1);
        for (let k = 0; k < count; k++)
            out.push(left + step * (k + 1));
    }
    else if (left !== null) {
        for (let k = 0; k < count; k++)
            out.push(left + (k + 1));
...
function runsToDelta(runs) {
    const ops = [];
    for (const run of runs) {
        if (!isPlainObject(run) || typeof run.text !== "string" || run.text.length === 0)
            continue;
        const { text, ...payload } = run;
        const attrs = { [RUN_ATTR_KEY]: cloneJson(payload) };
        const last = ops[ops.length - 1];
        if (last && deepEqual(last.attributes, attrs))
            last.insert += text;
        else
...
function deltaEqual(a, b) {
    ...
    for (let i = 0; i < a.length; i++) {
        if (a[i].insert !== b[i].insert)
            return false;
        if (!deepEqual(a[i].attributes ?? null, b[i].attributes ?? null))
            return false;
    }
    return true;
}
/** Expand a delta to a per-character attributes array of length `len`. */
function expandAttrs(delta, len) {
    const out = new Array(len);
    let i = 0;
    for (const op of delta) {
        if (typeof op.insert !== "string")
            continue;
        for (let k = 0; k < op.insert.length && i < len; k++)
            out[i++] = op.attributes;
    }
    return out;
}
 ...
 */
function reconcileRunsAsText(ytext, runs) {
    const desired = runsToDelta(runs);
    if (deltaEqual(ytext.toDelta(), desired))
        return;
    ...
    if (oldText !== newText) {
        ...
        let pre = 0;
        while (pre < cap && oldText[pre] === newText[pre])
            pre++;
        let suf = 0;
        while (suf < cap - pre && oldText[oldText.length - 1 - suf] === newText[newText.length - 1 - suf])
            suf++;
        const delLen = oldText.length - pre - suf;
        if (delLen > 0)
            ytext.delete(pre, delLen);
        const ins = newText.slice(pre, newText.length - suf);
        if (ins.length > 0)
            ytext.insert(pre, ins); // attributes applied by the format pass
    }
    // (2) Formatting reconciliation over the now length-matched text.
    const want = expandAttrs(desired, newText.length);
    const have = expandAttrs(ytext.toDelta(), newText.length);
    let i = 0;
 ...
 *  stays idempotent). */
function reconcileRuns(pmap, runs) {
    const existing = pmap.get("runs");
    const yt = existing instanceof Y.Text ? existing : new Y.Text();
    if (!(existing instanceof Y.Text))
        pmap.set("runs", yt);
    if (isCanonicalRuns(runs)) {
        reconcileRunsAsText(yt, runs);
        if (pmap.has(RUNS_STASH_KEY))
            pmap.delete(RUNS_STASH_KEY);
    }
    else {
        ...
        if (pmap.get(RUNS_STASH_KEY) !== next)
            pmap.set(RUNS_STASH_KEY, next);
    }
}
 ...
 *  structural path. */
function reconcileParagraphMap(pmap, para) {
    ...
    for (const key of [...pmap.keys()]) {
        if (key === RUNS_STASH_KEY)
            continue;
        if (!(key in para) || para[key] === undefined)
            pmap.delete(key);
    }
}
/** True when a live paragraph Y.Map projects deep-equal to a source paragraph. */
function paragraphEqual(live, src) {
 ...
 */
function reconcileParagraphArray(target, source) {
    const live = target.toArray();
    const liveLen = live.length;
    const srcLen = source.length;
    const cap = Math.min(liveLen, srcLen);
    let pre = 0;
    while (pre < cap && paragraphEqual(live[pre], source[pre]))
        pre++;
    let suf = 0;
    while (suf < cap - pre && paragraphEqual(live[liveLen - 1 - suf], source[srcLen - 1 - suf]))
        suf++;
    ...
    for (let k = 0; k < overlap; k++) {
        const pm = live[pre + k];
        if (pm instanceof Y.Map)
            reconcileParagraphMap(pm, source[pre + k]);
    }
 ...
 * deep-equals `doc`.
 */
export function reconcile(doc, ydoc) {
    const root = ydoc.getMap(DESIGN_ROOT_KEY);
    ydoc.transact(() => {
        reconcileMap(root, doc);
    }, LOCAL_ORIGIN);
}
/**
 ...
 */
export function fromDoc(ydoc) {
    return yToJson(ydoc.getMap(DESIGN_ROOT_KEY));
}
/**
 ...
 */
export function fromDocWithPageReuse(ydoc, reusable) {
    const root = ydoc.getMap(DESIGN_ROOT_KEY);
    const out = {};
...
function projectPagesWithReuse(pages, reusable) {
    ...
    const entries = raw.map((src) => {
        ...
        return { obj: cached ?? yToJson(src), rank: hasRank ? o : Number.POSITIVE_INFINITY, id: typeof id === "string" ? id : "" };
    });
 ...
 */
export function reconcileNodeMap(target, source) {
