...
exports.formToHtml = formToHtml;
exports.submissionsToCsv = submissionsToCsv;
...
function validateField(field, value) {
    if (field.kind === "submit" || field.kind === "hidden")
        return null;
...
function formToHtml(form, opts = {}) {
    ...
    const honeypotInput = `<div class="oc-hp" aria-hidden="true" style="position:absolute;left:-9999px">` +
        `<label>Leave this field empty<input type="text" name="${(0, html_1.escapeAttr)(honeypot)}" tabindex="-1" autocomplete="off"></label>` +
        `</div>`;
    ...
    const formIdInput = `<input type="hidden" name="oc_form_id" value="${(0, html_1.escapeAttr)(form.id)}">`;
    const after = form.afterSubmit.kind === "redirect"
        ? `<input type="hidden" name="oc_redirect" value="${(0, html_1.escapeAttr)(form.afterSubmit.url)}">`
        : "";
    return (`<form class="oc-form" data-oc-form="${(0, html_1.escapeAttr)(form.id)}" method="${method}" action="${(0, html_1.escapeAttr)(action)}" enctype="multipart/form-data" novalidate>` +
        honeypotInput +
        tokenInput +
        formIdInput +
        after +
...
function fieldToHtml(field) {
    const id = `oc-field-${(0, html_1.escapeAttr)(field.id)}`;
    const name = (0, html_1.escapeAttr)(field.name);
    const required = field.required ? " required" : "";
    const placeholder = field.placeholder ? ` placeholder="${(0, html_1.escapeAttr)(field.placeholder)}"` : "";
    const labelHtml = field.kind === "submit" || field.kind === "hidden"
        ? ""
        : `<label for="${id}">${(0, html_1.escapeHtml)(field.label || field.name)}${field.required ? " *" : ""}</label>`;
    let control;
    switch (field.kind) {
        case "textarea":
        ...
        case "select": {
            const opts = (field.options ?? [])
                .map((o) => `<option value="${(0, html_1.escapeAttr)(o)}">${(0, html_1.escapeHtml)(o)}</option>`)
                .join("");
        ...
        case "radio": {
            control = (field.options ?? [])
                .map((o, i) => `<label class="oc-radio"><input type="radio" name="${name}" value="${(0, html_1.escapeAttr)(o)}"${i === 0 ? required : ""}> ${(0, html_1.escapeHtml)(o)}</label>`)
                .join("");
            break;
        }
        case "checkbox":
        ...
        case "hidden":
            ...
            break;
        case "submit":
            control = `<button type="submit" class="oc-submit">${(0, html_1.escapeHtml)(field.label || "Submit")}</button>`;
            break;
        case "number":
            control = `<input type="number" id="${id}" name="${name}"${placeholder}${required}${field.validation?.min !== undefined ? ` min="${field.validation.min}"` : ""}${field.validation?.max !== undefined ? ` max="${field.validation.max}"` : ""}>`;
            break;
        case "email":
        ...
        case "text":
        default:
            control = `<input type="text" id="${id}" name="${name}"${placeholder}${required}${field.validation?.pattern ? ` pattern="${(0, html_1.escapeAttr)(field.validation.pattern)}"` : ""}>`;
    }
...
function submissionsToCsv(fields, submissions) {
    const cols = fields.filter((f) => f.kind !== "submit");
    const header = cols.map((f) => csvCell(f.name)).join(",");
    const rows = submissions.map((sub) => cols.map((f) => csvCell(sub.values[f.name])).join(","));
