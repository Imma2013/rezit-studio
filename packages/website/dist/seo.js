...
function metaTag(name, content) {
    if (!content)
        return "";
    return `<meta name="${(0, html_1.escapeAttr)(name)}" content="${(0, html_1.escapeAttr)(content)}">`;
}
function propTag(property, content) {
    if (!content)
        return "";
    return `<meta property="${(0, html_1.escapeAttr)(property)}" content="${(0, html_1.escapeAttr)(content)}">`;
}
...
function metaTags(site, page, ctx = {}) {
    ...
    const tags = [];
    if (title)
        tags.push(`<title>${(0, html_1.escapeAttr)(title)}</title>`);
    tags.push(metaTag("description", description));
    if (keywords && keywords.length > 0)
        tags.push(metaTag("keywords", keywords.join(", ")));
    tags.push(metaTag("robots", robots));
    if (canonical)
        tags.push(`<link rel="canonical" href="${(0, html_1.escapeAttr)(canonical)}">`);
    ...
    if (site.settings.faviconAssetId && ctx.resolveAssetUrl) {
        ...
        if (url)
            tags.push(`<link rel="icon" href="${(0, html_1.escapeAttr)(url)}">`);
    }
...
function jsonLd(site, page, ctx = {}) {
    ...
    const webPage = { "@type": "WebPage", name: title };
    if (description)
...
function sitemapXml(site, pages, baseUrl) {
    const slugs = (0, nav_1.pageSlugMap)(site, pages);
    ...
    for (const id of order) {
        ...
        if (slug === undefined)
            continue;
        const href = (0, nav_1.pageHref)(slug, id === site.homePageId);
        urls.push(`  <url>\n    <loc>${(0, html_1.escapeAttr)(joinUrl(baseUrl, href))}</loc>\n  </url>`);
    }
...
function robotsTxt(site, baseUrl) {
