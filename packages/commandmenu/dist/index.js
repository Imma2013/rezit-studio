...
function boost(id, opts) {
    let b = 0;
    const ri = opts.recents?.indexOf(id) ?? -1;
    if (ri >= 0)
        b += RECENCY_WEIGHT - ri; // more recent -> larger boost
 ...
 */
function search(query, items, opts = {}) {
    const q = query.trim();
    const scored = [];
    for (const item of items) {
        ...
        if (q.length === 0) {
            ...
            if (base === null)
                continue; // no match -> excluded from filtered results
        }
