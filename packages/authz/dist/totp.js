...
function hotp(secret, counter, digits = TOTP_DIGITS) {
    ...
    const hmac = (0, node_crypto_1.createHmac)("sha1", secret).update(buf).digest();
 ...
 */
function verifyTotp(secret, code, window = 1, time = Date.now()) {
    ...
    for (let i = -window; i <= window; i++) {
        if (safeEqual(hotp(key, counter + i), normalized))
            return true;
    }
    return false;
}
...
function generateRecoveryCodes(n = 10) {
    const codes = [];
    for (let i = 0; i < n; i++) {
        const raw = base32Encode((0, node_crypto_1.randomBytes)(8)).toLowerCase().slice(0, 12);
