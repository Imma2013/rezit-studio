...
exports.resolveBaseDirection = resolveBaseDirection;
exports.resolveLevels = resolveLevels;
exports.reorderRuns = reorderRuns;
exports.orderLinePieces = orderLinePieces;
...
function bidiClass(cp) {
    ...
    if (cp >= 0x0600 && cp <= 0x07bf) {
        if (cp >= 0x0660 && cp <= 0x0669)
            return "AN"; // Arabic-Indic digits
        if (cp >= 0x066b && cp <= 0x066c)
            return "AN"; // Arabic decimal/thousands separator
        if (cp >= 0x06f0 && cp <= 0x06f9)
            return "EN"; // Extended Arabic-Indic digits
    ...
    if (cp >= 0x0700 && cp <= 0x085f)
        return "R"; // Syriac, Thaana, N'Ko, Samaritan, Mandaic
    if (cp >= 0xfb1d && cp <= 0xfb4f)
        return "R"; // Hebrew presentation forms
    if (cp >= 0xfb50 && cp <= 0xfdff)
        return "AL"; // Arabic presentation forms A
    if (cp >= 0xfe70 && cp <= 0xfeff)
        return "AL"; // Arabic presentation forms B
    if (cp >= 0x0030 && cp <= 0x0039)
        return "EN"; // ASCII digits
    if (cp === 0x002b || cp === 0x002d)
        return "ES"; // plus, hyphen-minus
    if (cp === 0x0023 || cp === 0x0024 || cp === 0x0025)
        return "ET"; // # $ %
    if (cp >= 0x00a2 && cp <= 0x00a5)
        return "ET"; // currency signs
    if (cp === 0x002c || cp === 0x002e || cp === 0x003a || cp === 0x002f)
        return "CS"; // , . : /
    if (cp === 0x000a || cp === 0x000d || cp === 0x001c || cp === 0x001d || cp === 0x001e || cp === 0x0085)
        return "B";
    if (cp === 0x0009 || cp === 0x000b || cp === 0x001f)
        return "S";
    if (cp === 0x0020 || cp === 0x000c || cp === 0x2028 || cp === 0x2029)
        return "WS";
    ...
    if (cp >= 0x0061 && cp <= 0x007a)
        return "L";
    if (cp >= 0x00c0 && cp <= 0x058f)
        return "L"; // Latin-1 supplement through Armenian
    if (cp >= 0x0900 && cp <= 0x1fff)
        return "L"; // Indic, SE Asian, Georgian, Greek ext
    if (cp >= 0x2c00 && cp <= 0xd7ff)
        return "L"; // Coptic through Hangul
    if (cp >= 0xf900 && cp <= 0xfaff)
        return "L"; // CJK compatibility
    if (cp >= 0x10000)
        return "L"; // supplementary planes, minus the RTL blocks below
    return "ON";
}
 ...
 */
function resolveBaseDirection(text, declared = "auto") {
    if (declared === "ltr" || declared === "rtl")
        return declared;
    for (const ch of text) {
        const c = bidiClass(ch.codePointAt(0));
        if (c === "L")
            return "ltr";
        if (isStrongRtl(c))
            return "rtl";
 ...
 */
function resolveLevels(text, base) {
    ...
    for (let i = 1; i < n - 1; i++) {
        if (cls[i] === "ES" && cls[i - 1] === "EN" && cls[i + 1] === "EN")
            cls[i] = "EN";
        if (cls[i] === "CS" && cls[i - 1] === "EN" && cls[i + 1] === "EN")
            cls[i] = "EN";
        if (cls[i] === "CS" && cls[i - 1] === "AN" && cls[i + 1] === "AN")
            cls[i] = "AN";
    }
    ...
    const dirOf = (c) => c === "L" ? "L" : c === "R" || c === "EN" || c === "AN" ? "R" : null;
    for (let i = 0; i < n; i++) {
        if (dirOf(cls[i]) !== null)
            continue;
        let j = i;
        while (j < n && dirOf(cls[j]) === null)
            j++;
...
function reorderRuns(levels) {
    const runs = [];
    for (let i = 0; i < levels.length;) {
        let j = i;
        while (j < levels.length && levels[j] === levels[i])
            j++;
    ...
    for (let lvl = highest; lvl >= lowestOdd && lvl > 0; lvl--) {
        for (let i = 0; i < runs.length; i++) {
            if (runs[i].level < lvl)
                continue;
            let j = i;
            while (j < runs.length && runs[j].level >= lvl)
                j++;
 ...
 */
function orderLinePieces(items, base) {
    if (items.length === 0)
        return [];
    ...
    for (let i = 0; i < out.length;) {
        let j = i;
        while (j < out.length && out[j].level === out[i].level)
            j++;
...
function hasRtl(text) {
    for (const ch of text) {
        if (isStrongRtl(bidiClass(ch.codePointAt(0))))
            return true;
    }
    return false;
}
