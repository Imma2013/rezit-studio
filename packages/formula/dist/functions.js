...
function flattenAll(args) {
    const out = [];
    for (const a of args)
        out.push(...flattenArg(a));
    return out;
}
/** First error found in a value, or undefined. */
function findError(v) {
    if (isError(v))
        return v;
    if (Array.isArray(v)) {
...
exports.FUNCTIONS = {
    ...
    CONCATENATE(args, ctx) {
        return exports.FUNCTIONS.CONCAT(args, ctx);
    },
    LEN(args) {
    ...
    VLOOKUP(args) {
        ...
        if (!approximate) {
            for (const row of matrix) {
                if (looseEqual(row[0], lookup))
                    return row[colIndex] ?? ERR("#N/A");
            }
            return ERR("#N/A");
        }
    ...
    LOOKUP(args) {
        ...
        const resultVec = args.length > 2 ? flattenArg(args[2]) : searchVec;
        // largest value <= lookup
        let bestIdx = -1;
        for (let i = 0; i < searchVec.length; i++) {
            if (compareValues(searchVec[i], lookup) <= 0)
                bestIdx = i;
            else
                break;
        }
    ...
    SWITCH(args) {
        ...
        for (; i + 1 < args.length; i += 2) {
            if (looseEqual(subject, scalar(args[i])))
                return scalar(args[i + 1]);
        }
    ...
    SUBSTITUTE(args) {
        ...
        if (args.length > 3) {
            ...
            while ((idx = text.indexOf(o, from)) >= 0) {
                count++;
                if (count === which)
                    return text.slice(0, idx) + n + text.slice(idx + o.length);
                from = idx + o.length;
            }
            return text;
        }
    ...
    HLOOKUP(args) {
        ...
        if (!approx) {
            for (let c = 0; c < header.length; c++)
                if (looseEqual(header[c], lookup))
                    return matrix[rowIndex][c] ?? ERR("#N/A");
            return ERR("#N/A");
        }
        let best = -1;
        for (let c = 0; c < header.length; c++) {
            if (compareValues(header[c], lookup) <= 0)
                best = c;
            else
                break;
        }
    ...
    INDEX(args) {
        ...
        const ri = row - 1;
        const ci = col - 1;
        if (ri < 0 || ri >= matrix.length || ci < 0 || ci >= matrix[ri].length)
            return ERR("#REF!");
...
function criteriaPredicate(criterion) {
    if (isError(criterion))
        return () => false;
    ...
    const rhs = rhsIsNum ? rhsNum : rhsRaw;
    return (v) => {
        if (isError(v))
            return false;
        if (op === "" || op === "=")
            return rhsIsNum && typeof v === "number" ? v === rhsNum : looseEqual(v, rhs);
        if (op === "<>")
            return rhsIsNum && typeof v === "number" ? v !== rhsNum : !looseEqual(v, rhs);
