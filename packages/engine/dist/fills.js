...
exports.resolveFill = resolveFill;
const color_1 = require("./color");
function firstStopColor(fill) {
    if (fill.stops.length > 0)
        return (0, color_1.colorToCss)(fill.stops[0].color);
    if (fill.mesh && fill.mesh.points.length > 0)
        return (0, color_1.colorToCss)(fill.mesh.points[0].color);
    return "rgba(0, 0, 0, 0)";
}
function addStops(g, stops) {
    ...
    for (const s of stops) {
        ...
        g.addColorStop(p, (0, color_1.colorToCss)(s.color));
    }
}
const finite = (x) => Number.isFinite(x);
function resolveGradient(ctx, fill, w, h) {
    ...
    if (fill.gradient === "linear") {
        if (!ctx.createLinearGradient)
            return firstStopColor(fill);
        const angle = finite(fill.angle) ? fill.angle : 0;
    ...
    if (fill.gradient === "radial") {
        ...
        if (!ctx.createRadialGradient || !(r > 0))
            return firstStopColor(fill);
        const cx = center.x * w;
        const cy = center.y * h;
...
function resolveFill(ctx, fill, w, h) {
    switch (fill.type) {
        case "solid":
            return (0, color_1.colorToCss)(fill.color);
        case "gradient":
