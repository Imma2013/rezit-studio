...
exports.bumpTextLayout = bumpTextLayout;
exports.blendToComposite = blendToComposite;
exports.renderScene = renderScene;
const schema_1 = require("@hc/schema");
const schema_2 = require("@hc/schema");
const image_1 = require("./image");
const booleanGeom_1 = require("./booleanGeom");
const pathclip_1 = require("./pathclip");
const layer_1 = require("./layer");
const maskedImage_1 = require("./maskedImage");
const text_1 = require("@hc/text");
const color_1 = require("./color");
const fonts_1 = require("./fonts");
const effects_1 = require("./effects");
const duotone_1 = require("./duotone");
const fills_1 = require("./fills");
const chart_1 = require("./chart");
const color_2 = require("@hc/color");
...
function placeholderBox(ctx, w, h, fill = PLACEHOLDER_FILL, stroke = PLACEHOLDER_STROKE) {
    ctx.fillStyle = fill;
...
function setStroke(ctx, width, style) {
    ctx.lineWidth = width;
...
function paintPattern(ctx, pat, w, h, assets) {
    ...
    if (pattern.setTransform && typeof DOMMatrix !== "undefined") {
        ...
        try {
            pattern.setTransform(new DOMMatrix([cos * s, sin * s, -sin * s, cos * s, 0, 0]));
        }
        catch { /* unsupported: native scale */ }
    }
    ctx.fillStyle = pattern;
    ctx.fillRect(0, 0, w, h);
    return true;
}
function drawShape(ctx, node, assets) {
    const { width: w, height: h } = node.size;
    // Pattern fill (tiled image): clip to the shape outline, then tile the asset.
    const patFill = node.fills.find((f) => f.type === "pattern");
    if (patFill && ctx.save && ctx.restore && ctx.clip && ctx.createPattern) {
        ...
        if (node.stroke) {
            shapePath(ctx, node);
            setStroke(ctx, node.stroke.width, (0, fills_1.resolveFill)(ctx, node.stroke.fill, w, h));
        }
        return;
    }
    ...
    if (imgFill && ctx.save && ctx.restore && ctx.clip) {
        ...
        if (img && ctx.drawImage) {
            ...
            const fr = (0, image_1.fitRect)(iw * crop.width, ih * crop.height, w, h, imgFill.fit ?? "cover", imgFill.focalPoint);
    ...
    const fillStyle = node.fills.length > 0 ? (0, fills_1.resolveFill)(ctx, node.fills[0], w, h) : null;
    if (fillStyle !== null)
        ...
        case "custom": {
            ...
            if (cr && hasRadius && ctx.roundRect) {
                ...
                if (node.stroke) {
                    ...
                    ctx.strokeStyle = (0, fills_1.resolveFill)(ctx, node.stroke.fill, w, h);
                    ctx.stroke();
                }
            }
            else {
...
function drawMedia(ctx, w, h, assetId, assets) {
    const status = assets ? assets.status(assetId) : "loading";
    if (status === "ready") {
 ...
 */
function drawImageNode(ctx, node, w, h, assets) {
    ...
    if (status === "ready") {
        ...
        if (img && ctx.drawImage) {
            ...
            const duo = (0, effects_1.duotoneEffect)(node.effects);
            if (duo && duo.intensity > 0) {
                const mapped = (0, duotone_1.duotoneCanvas)(assetId, img, natW, natH, duo.shadows, duo.highlights, duo.intensity);
                if (mapped) {
                    img = mapped;
                    const mw = mapped.width;
                    const mh = mapped.height;
                    if (mw && mh) {
                        srcW = mw;
                        srcH = mh;
                    }
            ...
            const maskRef = node.alphaMask;
            if (maskRef && assets?.status(maskRef.assetId) === "ready") {
                const maskImg = assets.image(maskRef.assetId);
                if (maskImg) {
                    const masked = (0, maskedImage_1.maskedCanvas)(`${assetId}:${maskRef.assetId}:${srcW}x${srcH}`, img, maskImg, srcW, srcH);
                    if (masked) {
                        img = masked;
                        const mw = masked.width;
                        const mh = masked.height;
                        if (mw && mh) {
                            srcW = mw;
                            srcH = mh;
                        }
            ...
            const fr = (0, image_1.fitRect)(srcW * crop.width, srcH * crop.height, w, h, node.fit, node.focalPoint);
...
function connectorAnchorPoint(box, anchor, toward) {
...
function drawCap(ctx, cap, from, tip, width, color) {
    if (!cap || cap.kind === "none")
        return;
 ...
 */
function drawConnector(ctx, node, boxes, origin) {
    const conn = node;
    ...
    if (!wps)
        curved = conn.route === "curved" && pts.length > 2;
    // Shift page-space points into the connector node's local space.
    const lx = (p) => p.x - origin.x;
    const ly = (p) => p.y - origin.y;
    const width = conn.stroke?.width ?? 2;
    const color = (0, fills_1.resolveFill)(ctx, conn.stroke.fill, 1, 1);
    ctx.beginPath();
    ...
    if (conn.startCap)
        drawCap(ctx, conn.startCap, { x: lx(startNeighbor), y: ly(startNeighbor) }, { x: lx(a), y: ly(a) }, width, color);
    if (conn.endCap)
        drawCap(ctx, conn.endCap, { x: lx(endNeighbor), y: ly(endNeighbor) }, { x: lx(b), y: ly(b) }, width, color);
...
function orthogonalChain(points) {
    if (points.length === 0)
        return [];
    const out = [points[0]];
...
function drawInk(ctx, node) {
    ...
    const radiusAt = (i) => {
        if (!isPen)
            return baseR; // marker / highlighter: flat, constant nib
        const pr = pts[i].p;
    ...
    ctx.fillStyle = (0, color_1.colorToCss)(node.brush.color);
    if (pts.length === 1) {
 ...
 */
function traceVectorPath(ctx, path) {
...
function drawNodeContent(ctx, node, assets, boxes, origin) {
    const { width: w, height: h } = node.size;
    ...
    if ((0, schema_1.isUnknownNode)(node)) {
        placeholderBox(ctx, w, h);
        return;
    }
    switch (node.type) {
        case "text": {
            ...
            const measure = ctx.measureText
                ? (text, style) => {
                    ctx.font = (0, fonts_1.canvasFontString)(style);
                    ...
                    if ("letterSpacing" in ctx)
                        lctx.letterSpacing = `${style.letterSpacing ?? 0}px`;
            ...
            const hl = (0, schema_2.enabledTextEffects)(node.textEffects).find((e) => e.kind === "highlight");
            const hlFill = hl ? (0, fills_1.resolveFill)(ctx, hl.color, w, h) : null;
            ...
            const outline = (0, schema_2.enabledEffects)(node.effects).find((e) => e.kind === "outline");
            if (outline && "lineJoin" in ctx)
                ctx.lineJoin = "round";
            ...
            const tfx = (0, schema_2.enabledTextEffects)(node.textEffects);
            const eShadow = tfx.find((e) => e.kind === "shadow");
            const eLift = tfx.find((e) => e.kind === "lift");
            const eGlow = tfx.find((e) => e.kind === "glow");
            const eNeon = tfx.find((e) => e.kind === "neon");
            const eEcho = tfx.find((e) => e.kind === "echo");
            const eHollow = tfx.find((e) => e.kind === "hollow");
            const eSplice = tfx.find((e) => e.kind === "splice");
            const eOutlineFx = tfx.find((e) => e.kind === "outline");
            ...
            const flatColorAlpha = (fill, mult) => {
                ...
                return (0, color_1.colorToCss)({ srgb: { ...c.srgb, a: (c.srgb.a ?? 1) * m } });
            };
            const flatColor = (fill) => flatColorAlpha(fill, 1);
            const sctx = ctx;
            const hasShadowApi = "shadowBlur" in ctx;
            const clearShadow = () => { if (hasShadowApi) {
            ...
            const segWidth = (s) => {
                const t = (0, fonts_1.applyTextCase)(s.text, s.style.case);
            ...
            const segAdvance = (s, pos, tabStops) => {
                if ((0, text_1.isTabRun)(s.text))
                    return (0, text_1.tabRunWidth)(s.text, pos, tabStops, s.style.fontSize);
            ...
            const lineAdvance = (segs, tabStops) => {
                let pos = 0;
                for (const s of segs)
                    pos += segAdvance(s, pos, tabStops);
                return pos;
            };
            ...
            const { lines } = (0, text_1.layoutText)((0, text_1.autoFitNode)(node, measure ? { measure } : {}), measure ? { measure } : {});
            ...
            const isWs = (t) => t.length > 0 && t.trim() === "";
            ...
            for (let li = 0; !arcCurv && li < lines.length; li++) {
                ...
                if (line.align === "justify" && !lastOfPara && !line.marker) {
                    ...
                    const ws = line.segments.filter((s) => isWs(s.text) && !(0, text_1.isTabRun)(s.text)).length;
                    const slack = (colLeft + colW) - (colLeft + line.x) - lineWidth;
                    if (ws > 0 && slack > 0)
                        justifyExtra = slack / ws;
                }
                ...
                const startX = line.marker || align === "left"
                    ? colLeft + line.x
                    : align === "center"
                ...
                if (hl && hlFill) {
                    ...
                    for (let k = line.segments.length - 1; k >= 0; k--) {
                        if (isWs(line.segments[k].text))
                            visW -= segWidth(line.segments[k]);
                        else
                            break;
                    }
                    if (visW > 0) {
                ...
                if (line.marker) {
                    ctx.font = (0, fonts_1.canvasFontString)(line.marker.style);
                    ctx.fillStyle = line.marker.style.fill.type === "solid" ? (0, color_1.colorToCss)(line.marker.style.fill.color) : "rgba(0,0,0,1)";
                ...
                for (const seg of line.segments) {
                    ...
                    ctx.font = (0, fonts_1.canvasFontString)(drawStyle);
                    if ("letterSpacing" in ctx)
                        lctx.letterSpacing = `${seg.style.letterSpacing ?? 0}px`;
                    ...
                    ctx.textRendering = seg.style.features?.liga === 0 ? "optimizeSpeed" : "auto";
                    const t = (0, fonts_1.applyTextCase)(seg.text, seg.style.case);
                    ...
                    const segCss = seg.style.fill.type === "solid"
                        ? (0, color_1.colorToCss)(seg.style.fill.color)
                        : seg.style.fill.type === "gradient"
                            ? (0, fills_1.resolveFill)(ctx, seg.style.fill, w, h)
                            : "rgba(0,0,0,1)";
                    if (!isWs(seg.text)) {
                        ...
                        if (eEcho && ctx.fillText) {
                            ...
                            for (let i = n; i >= 1; i--) {
                                ...
                                if (typeof g.globalAlpha === "number")
                        ...
                        if (outline && ctx.strokeText) {
                            ...
                            ctx.strokeStyle = (0, color_1.colorToCss)(outline.color);
                            ctx.strokeText(t, x, baseY);
                        }
                        if (eOutlineFx && ctx.strokeText) {
                            ctx.lineWidth = Math.max(0.5, eOutlineFx.width);
                            ctx.strokeStyle = (0, fills_1.resolveFill)(ctx, eOutlineFx.color, w, h);
                            ctx.strokeText(t, x, baseY);
                        }
                        if (eSplice && ctx.strokeText) {
                            const o = eSplice.offset || 0;
                            ctx.lineWidth = Math.max(0.5, eSplice.thickness);
                            ctx.strokeStyle = flatColor(eSplice.color);
                            ctx.strokeText(t, x + o, baseY + o);
                        }
                        // Glow / neon / shadow / lift cast via the canvas drop shadow.
                        if (hasShadowApi) {
                            if (eGlow) {
                                const gi = Math.max(0.2, eGlow.intensity || 1);
                                sctx.shadowColor = flatColorAlpha(eGlow.color, gi);
                                sctx.shadowBlur = Math.max(0, eGlow.radius);
                                sctx.shadowOffsetX = 0;
                                sctx.shadowOffsetY = 0;
                            }
                            else if (eNeon) {
                                sctx.shadowColor = flatColor(eNeon.color);
                                sctx.shadowBlur = Math.max(4, fs * 0.5 * Math.max(0.2, eNeon.intensity || 1));
                                sctx.shadowOffsetX = 0;
                                sctx.shadowOffsetY = 0;
                            }
                            else if (eShadow) {
                                sctx.shadowColor = flatColorAlpha(eShadow.color, eShadow.opacity ?? 1);
                                sctx.shadowBlur = Math.max(0, eShadow.blur);
                                sctx.shadowOffsetX = eShadow.dx;
                                sctx.shadowOffsetY = eShadow.dy;
                            }
                            else if (eLift) {
                                const k = Math.max(0.1, eLift.intensity || 0.5);
                                sctx.shadowColor = `rgba(0,0,0,${0.35 * k})`;
                                sctx.shadowBlur = fs * 0.3 * k + 2;
                                sctx.shadowOffsetX = 0;
                                sctx.shadowOffsetY = fs * 0.06;
                            }
                        }
                        if (fillVisible && ctx.fillText) {
                            ...
                            if (eGlow && hasShadowApi && (eGlow.intensity ?? 1) > 1) {
                                for (let gp = Math.min(2, Math.round(eGlow.intensity) - 1); gp > 0; gp--)
                                    ctx.fillText(t, x, baseY);
                            }
                        }
                        clearShadow();
                        // Neon: a crisp second pass so the bright core reads over the glow.
                        if (eNeon && fillVisible && ctx.fillText) {
                            ctx.fillStyle = segCss;
                            ctx.fillText(t, x, baseY);
                        }
                        // Hollow: glyph outline only (no fill drawn above).
                        if (eHollow && ctx.strokeText) {
                            ctx.lineWidth = Math.max(0.5, eHollow.thickness || Math.max(1, fs / 16));
                    ...
                    x += stepW + (justifyExtra && isWs(seg.text) && !(0, text_1.isTabRun)(seg.text) ? justifyExtra : 0);
                }
            }
            ...
            if (arcCurv && ctx.save && ctx.restore && ctx.fillText) {
                ...
                for (const p of node.content)
                    for (const run of p.runs) {
                        for (const ch of Array.from((0, fonts_1.applyTextCase)(run.text, run.style.case)))
                            glyphs.push({ ch, style: run.style });
                    }
                ...
                if (W > 0) {
                    ...
                    if (hl && hlFill && ctx.arc && ctx.beginPath && ctx.stroke) {
                        ...
                        const cctx = ctx;
                        if ("lineCap" in ctx)
                            cctx.lineCap = hlRad > 0 ? "round" : "butt";
                        ctx.beginPath();
                    ...
                    for (let i = 0; i < glyphs.length; i++) {
                        ...
                        const worldOff = (o) => ({ x: o * (Math.cos(rot) + Math.sin(rot)), y: o * (Math.cos(rot) - Math.sin(rot)) });
                        if (eEcho) {
                            const ecol = flatColor(eEcho.color);
                            const n = Math.max(1, Math.min(8, Math.round(eEcho.count) || 3));
                            const off = eEcho.offset || fs * 0.12;
                            ...
                            for (let e = n; e >= 1; e--) {
                                ...
                                if (typeof gctx.globalAlpha === "number")
                                    gctx.globalAlpha = prev * a;
                                const d = worldOff(off * e);
                                ctx.fillStyle = ecol;
                                ctx.fillText(g.ch, d.x, d.y);
                                if (typeof gctx.globalAlpha === "number")
                                    gctx.globalAlpha = prev;
                            }
                        }
                        if (outline && ctx.strokeText) {
                            ctx.lineWidth = outline.width;
                        ...
                        if (eOutlineFx && ctx.strokeText) {
                            ctx.lineWidth = Math.max(0.5, eOutlineFx.width);
                            ctx.strokeStyle = flatColor(eOutlineFx.color);
                            ctx.strokeText(g.ch, 0, 0);
                        }
                        if (eSplice && ctx.strokeText) {
                            const d = worldOff(eSplice.offset || 0);
                            ctx.lineWidth = Math.max(0.5, eSplice.thickness);
                            ctx.strokeStyle = flatColor(eSplice.color);
                            ctx.strokeText(g.ch, d.x, d.y);
                        }
                        if (hasShadowApi) {
                            if (eGlow) {
                                const gi = Math.max(0.2, eGlow.intensity || 1);
        ...
        case "line": {
            const pts = node.points;
            if (pts.length >= 2) {
                const col = (0, fills_1.resolveFill)(ctx, node.stroke.fill, w, h);
                ctx.beginPath();
                ctx.moveTo(pts[0].x, pts[0].y);
                for (let i = 1; i < pts.length; i++)
                    ctx.lineTo(pts[i].x, pts[i].y);
                setStroke(ctx, node.stroke.width, col);
                if (node.endCap === "arrow")
                    arrowhead(ctx, pts[pts.length - 2], pts[pts.length - 1], node.stroke.width, col);
                if (node.startCap === "arrow")
                    arrowhead(ctx, pts[1], pts[0], node.stroke.width, col);
            }
            break;
        }
        case "path": {
            ...
            if (segs.length >= 2 || contours.length) {
                ...
                ctx.beginPath();
                if (segs.length >= 2)
                    trace(segs, node.closed);
                for (const c of contours)
                    trace(c.segments, c.closed);
                if (node.fills && node.fills.length > 0) {
                    ctx.fillStyle = (0, fills_1.resolveFill)(ctx, node.fills[0], w, h);
                    if (contours.length)
                        ctx.fill("evenodd");
                    else
                        ctx.fill();
        ...
        case "boolean": {
            ...
            const r = node.result && node.result.subpaths.length > 0 ? node.result : (0, booleanGeom_1.booleanGeometry)(node);
        ...
        case "sticky": {
            ...
            const clamp01 = (v) => Math.max(0, Math.min(1, v));
            const rgba = (r, g, b, a) => `rgba(${Math.round(clamp01(r) * 255)},${Math.round(clamp01(g) * 255)},${Math.round(clamp01(b) * 255)},${a})`;
            ...
            let cardFill = (0, fills_1.resolveFill)(ctx, node.fill, w, h);
            ...
            if (node.text) {
                ...
                ctx.fillStyle = (0, color_1.colorToCss)(node.textColor);
                const maxW = Math.max(1, w - pad * 2);
                const measure = (s) => (ctx.measureText ? ctx.measureText(s).width : s.length * fontPx * 0.55);
...
function drawStamp(ctx, node, w, h) {
    const s = node;
    const glyph = s.glyph || "👍";
...
function drawQr(ctx, node, w, h, assets) {
    const qr = node;
    const m = qr.modules;
    if (!m || !m.length) {
        placeholderBox(ctx, w, h);
    ...
    ctx.fillStyle = qr.background ? (0, color_1.colorToCss)(qr.background) : "#ffffff";
    ctx.fillRect(0, 0, w, h);
    ctx.fillStyle = qr.foreground ? (0, color_1.colorToCss)(qr.foreground) : "#000000";
    for (let r = 0; r < n; r++) {
        for (let c = 0; c < n; c++) {
            if (m[r][c])
...
function arrowhead(ctx, from, tip, width, color) {
    const a = Math.atan2(tip.y - from.y, tip.x - from.x);
...
function drawTable(ctx, node, w, h) {
    const xs = [0];
    for (const cw of node.colWidths)
        xs.push(xs[xs.length - 1] + cw);
    const ys = [0];
    for (const rh of node.rowHeights)
        ys.push(ys[ys.length - 1] + rh);
    ...
    const cellNum = (c) => parseFloat(String(c.content?.[0]?.text ?? "").replace(/[,%$\s]/g, ""));
    // Cache per-scope numeric ranges (column index, or -1 for the whole data area).
    const rangeCache = new Map();
    const rangeFor = (column) => {
        const key = column ?? -1;
        ...
        for (const c of node.cells) {
            if (c.row < headerRows)
                continue;
            if (column !== undefined && c.col !== column)
                continue;
            const n = cellNum(c);
    ...
    const cmp = (a, op, b) => op === ">" ? a > b : op === "<" ? a < b : op === ">=" ? a >= b : op === "<=" ? a <= b : op === "==" ? a === b : a !== b;
    for (const cell of node.cells) {
        ...
        if (!isHeader && rules.length) {
            ...
            if (Number.isFinite(val)) {
                for (const rule of rules) {
                    if (rule.column !== undefined && rule.column !== cell.col)
                        continue;
                    if (rule.kind === "highlight") {
                        if (cmp(val, rule.op, rule.value)) {
                            if (rule.fill)
                                ruleFill = (0, fills_1.resolveFill)(ctx, rule.fill, cw, ch);
                            if (rule.textColor)
                                ruleText = rule.textColor;
                        }
                    }
                    else if (rule.kind === "colorScale") {
                        const { min, max } = rangeFor(rule.column);
                        const t = max > min ? (val - min) / (max - min) : 0;
                        ruleFill = (0, color_1.colorToCss)(lerpColor(rule.min, rule.max, t));
                    }
                    else {
                        const { min, max } = rangeFor(rule.column);
                        const t = max > min ? (val - min) / (max - min) : 0;
                        dataBar = { css: (0, color_1.colorToCss)(rule.color), frac: Math.max(0, Math.min(1, t)) };
                    }
                }
            }
        }
        ...
        if (ruleFill) {
            ...
            ctx.fillStyle = (0, fills_1.resolveFill)(ctx, baseFill, cw, ch);
            ctx.fillRect(cx, cy, cw, ch);
        }
        if (dataBar) {
            ctx.save();
        ...
        if (run?.text) {
            ...
            ctx.font = `${run.italic ? "italic " : ""}${weight} ${size}px ${(0, fonts_1.fontFamilyStack)("system")}`;
            ...
            ctx.fillStyle = textColor ? (0, color_1.colorToCss)(textColor) : "#18181b";
    ...
    if (bs && bs.show === false)
        return; // borders explicitly hidden
    const borderColor = bs?.color ? (0, color_1.colorToCss)(bs.color) : node.borders ? (0, fills_1.resolveFill)(ctx, node.borders.fill, totalW, totalH) : "#d4d4d8";
    ...
    for (const gy of ys) {
        ctx.moveTo(0, gy);
        ctx.lineTo(totalW, gy);
    }
    ctx.stroke();
}
function seriesColor(s, i) {
    return s.color ? (0, color_1.colorToCss)(s.color) : color_2.SERIES_PALETTE_HEX[i % color_2.SERIES_PALETTE_HEX.length];
}
...
function valueLabel(ctx, text, x, y, k = 1) {
    ctx.font = `500 ${Math.round(10 * k)}px ${(0, fonts_1.fontFamilyStack)("system")}`;
...
function ellipsize(ctx, text, maxWidth) {
    if (maxWidth <= 0)
        return "";
    if (textWidth(ctx, text) <= maxWidth)
        return text;
    const ell = "…";
    let lo = 0, hi = text.length;
    while (lo < hi) {
        const mid = Math.ceil((lo + hi) / 2);
        if (textWidth(ctx, text.slice(0, mid) + ell) <= maxWidth)
            lo = mid;
        else
            hi = mid - 1;
    }
...
function drawLegend(ctx, node, w, h, position) {
    const series = node.series ?? [];
    if (series.length === 0)
        return;
    const k = chartTextScale(node);
    ctx.font = `500 ${Math.round(11 * k)}px ${(0, fonts_1.fontFamilyStack)("system")}`;
    ctx.textAlign = "left";
    ...
    if (position === "top" || position === "bottom") {
        ...
        for (let j = 0; j < series.length; j++) {
            if (x + sw + gap >= maxX)
                break; // no room left for another item
...
function drawYAxis(ctx, x0, y0, ph, maxV, k = 1) {
    const ticks = (0, chart_1.tickCount)(ph);
    ...
    ctx.lineTo(x0, y0 + ph);
    ctx.stroke();
    ctx.font = `500 ${Math.round(9 * k)}px ${(0, fonts_1.fontFamilyStack)("system")}`;
    ctx.fillStyle = "#52525b";
    ctx.textAlign = "right";
...
function chartInsets(node, w, h) {
    ...
    if (style?.axes?.showY !== false && hasValueAxis(node.chartType))
        left += 22 * k;
...
function drawChartChrome(ctx, node, w, h) {
    const style = node.style;
    if (!style)
        return;
    const k = chartTextScale(node);
    if (style.title) {
        ctx.font = `600 ${Math.round(13 * k)}px ${(0, fonts_1.fontFamilyStack)("system")}`;
...
function drawChart(ctx, node, w, h) {
    ...
    if (type === "pie" || type === "donut") {
        ...
        for (let i = 0; i < vals.length; i++) {
            ...
            const col = color_2.SERIES_PALETTE_HEX[i % color_2.SERIES_PALETTE_HEX.length];
            if (type === "donut") {
                ctx.beginPath();
    ...
    const { x0, y0, pw, ph } = chartInsets(node, w, h);
    const n = (0, chart_1.categoryCount)(node.categories ?? [], series);
    if (n === 0) {
        placeholderBox(ctx, w, h);
        return;
    }
    ...
    const maxV = (stacked ? (0, chart_1.stackedMax)(series, n) : (0, chart_1.seriesMax)(series)) || 1;
    ...
    if (node.style?.axes?.showY !== false)
        drawYAxis(ctx, x0, y0, ph, maxV, k);
        ...
        for (let i = 0; i < n; i++) {
            if (stacked) {
                ...
                for (let j = 0; j < series.length; j++) {
                    ...
                    const bh = (v / maxV) * ph;
                    const base = (0, chart_1.stackedBase)(series, i, j);
                    const by = y0 + ph - ((base + v) / maxV) * ph;
                    ctx.fillStyle = seriesColor(series[j], j);
                    ctx.fillRect(bx, by, barW, bh);
                    if (showValues && v > 0)
                        valueLabel(ctx, String(series[j].values[i]), bx + barW / 2, by + 10 * k, k);
                }
            }
            else {
                for (let j = 0; j < series.length; j++) {
                    ...
                    const g = (0, chart_1.groupedBarLayout)(pw, n, series.length, i, j);
                    ctx.fillStyle = seriesColor(series[j], j);
                    ctx.fillRect(x0 + g.x, y0 + ph - bh, g.width * 0.9, bh);
                    if (showValues)
                        valueLabel(ctx, String(v), x0 + g.x + g.width * 0.45, y0 + ph - bh - 3 * k, k);
                }
            }
        }
        return;
...
function drawScatter(ctx, node, x0, y0, pw, ph, n, showValues) {
    const series = node.series ?? [];
    const maxV = (0, chart_1.seriesMax)(series) || 1;
    const k = chartTextScale(node);
    if (node.style?.axes?.showX !== false) {
        ctx.strokeStyle = "#d4d4d8";
        ctx.lineWidth = 1;
    ...
    for (let j = 0; j < series.length; j++) {
        ...
        for (let i = 0; i < series[j].values.length; i++) {
            ...
            if (showValues)
                valueLabel(ctx, String(v), px, py - 6 * k, k);
        }
    }
}
/** Radar/spider: each series is a closed polygon over the category axes. */
function drawRadar(ctx, node, cx, cy, radius, n) {
    const series = node.series ?? [];
    const maxV = (0, chart_1.seriesMax)(series) || 1;
    // axis web
    ...
    ctx.lineWidth = 1;
    for (let i = 0; i < n; i++) {
        const p = (0, chart_1.radarPoint)(cx, cy, radius, i, n, maxV, maxV);
        ctx.beginPath();
        ctx.moveTo(cx, cy);
        ctx.lineTo(p.x, p.y);
        ctx.stroke();
    ...
    for (let j = 0; j < series.length; j++) {
        ...
        for (let i = 0; i < n; i++) {
            const v = series[j].values[i] ?? 0;
            const p = (0, chart_1.radarPoint)(cx, cy, radius, i, n, v, maxV);
            if (i === 0)
                ctx.moveTo(p.x, p.y);
            else
                ctx.lineTo(p.x, p.y);
 ...
 */
function paintIsolated(ctx, sn, alpha, opts, boxes, cull) {
    if (!ctx.getTransform || !ctx.canvas || !ctx.drawImage || !ctx.save || !ctx.restore)
        return false;
    const { width, height } = ctx.canvas;
    const layer = (0, layer_1.makeLayerCanvas)(width, height);
    if (!layer)
        return false;
    const lctx = (0, layer_1.layerContext)(layer);
    if (!lctx)
        return false;
    const t = ctx.getTransform();
    lctx.setTransform(t.a, t.b, t.c, t.d, t.e, t.f);
    for (const child of sn.children)
        paint(lctx, child, 1, opts, boxes, cull);
...
function paint(ctx, sn, parentAlpha, opts, boxes, cull) {
    const node = sn.node;
    if (node.hidden || node.id === opts.skipNodeId || opts.hiddenIds?.has(node.id))
        return;
    const { width: w, height: h } = node.size;
    ...
    if (cull && !(0, schema_1.isContainer)(node) && node.type !== "connector") {
        const b = sn.worldBounds;
        const offscreen = b.x > cull.x + cull.w || b.x + b.width < cull.x || b.y > cull.y + cull.h || b.y + b.height < cull.y;
        if (offscreen || Math.max(b.width, b.height) * cull.zoom < 0.5)
            return;
    }
    ctx.save();
    const m = (0, math_1.fromTransform)(node.transform);
    ...
    if ((0, schema_1.isContainer)(node) && node.clip) {
        ...
        if (fr.maskShape === "custom" && fr.maskPath) {
            clipped = (0, pathclip_1.buildClipFromPathData)(ctx, fr.maskPath, w, h); // falls through to rect if unparseable
        }
        if (clipped) {
    ...
    const filter = (0, effects_1.effectsFilter)(node.effects);
    if (supportsFilter && filter !== "none")
        ctx.filter = filter;
    try {
    ...
    if (node.type !== "text") {
        for (const o of (0, effects_1.outlineSpecs)(node.effects)) {
    ...
    if (sn.children) {
        ...
        if (node.type !== "mask" && (0, layer_1.needsIsolation)(node, sn.children.length)) {
        ...
        if (node.type === "mask" && ctx.save && ctx.restore && ctx.clip) {
            const shape = node.maskShape;
            ctx.save();
            ctx.beginPath();
            if (traceVectorPath(ctx, shape)) {
                const rule = node.maskShape?.fillRule;
                try {
                    ctx.clip(rule === "evenodd" ? "evenodd" : "nonzero");
                }
                catch {
                    ctx.clip();
                }
        ...
        for (const child of sn.children)
            paint(ctx, child, alpha, opts, boxes, cull);
        if (clipped)
            ctx.restore();
    }
    ctx.restore();
...
function renderScene(scene, ctx, viewport, opts = {}) {
    ...
    if (!opts.skipBackground && page.background) {
        ctx.fillStyle = (0, fills_1.resolveFill)(ctx, page.background, page.width, page.height);
    ...
    if (kids) {
        ...
        for (const child of kids) {
            ...
            const inBounds = !(0, schema_1.isContainer)(n) && axisAligned && !!b &&
