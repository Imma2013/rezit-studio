...
exports.effectBleed = effectBleed;
...
function effectBleed(node) {
    ...
    const effects = (0, schema_2.enabledEffects)(node.effects);
    if (effects.length === 0)
        return 0;
    let bleed = 0;
    for (const e of effects) {
        switch (e.kind) {
            ...
            case "outline":
                ...
                break;
            case "shadow":
                bleed = Math.max(bleed, Math.abs(e.offsetX) + e.blur * SHADOW_HALO + e.spread, Math.abs(e.offsetY) + e.blur * SHADOW_HALO + e.spread);
                break;
            default:
                break;
        }
...
function computeWorldBounds(node, world) {
    const inflated = (0, math_1.rectInflate)(localBox(node), effectBleed(node) + strokeBleed(node));
    return (0, math_1.transformRect)(world, inflated);
}
// The active page is modeled as an implicit root group so the whole subtree is
// one uniform SceneNode tree.
function pageRootNode(file, pageIndex) {
...
function buildSceneNode(node, parentWorld, parent, index) {
    const world = (0, math_1.multiply)(parentWorld, (0, math_1.fromTransform)(node.transform));
    const sn = {
        node,
        worldTransform: (0, math_1.matToArray)(world),
    ...
    const nested = (0, schema_1.isContainer)(node)
        ? (0, schema_1.childrenOf)(node)
        : node.type === "mask"
            ? (0, schema_1.childNodesOf)(node)
            : [];
    if (nested.length > 0) {
...
function assetIdsOf(node) {
    const ids = [];
    // Image nodes hold their asset under `source.assetId`; others use `assetId`.
    if (node.type === "image") {
        const src = node.source;
        if (typeof src?.assetId === "string" && src.assetId.length > 0)
            ids.push(src.assetId);
        ...
        // no clue why.
        const mask = node.alphaMask;
        if (typeof mask?.assetId === "string" && mask.assetId.length > 0)
            ids.push(mask.assetId);
    }
    else {
        const direct = node.assetId;
        if (typeof direct === "string" && direct.length > 0)
            ids.push(direct);
    }
    const logo = node.logoAssetId;
    if (typeof logo === "string" && logo.length > 0)
        ids.push(logo);
    return ids;
}
function connectorAnchor(box, anchor, toward) {
...
function connectorEndRefCenter(ep, boxes) {
...
function resolveConnectorEndpoint(ep, boxes, toward) {
    if (ep.attach && boxes[ep.attach.nodeId])
        return connectorAnchor(boxes[ep.attach.nodeId], ep.attach.anchor, toward);
...
function connectorPolyline(conn, boxes) {
...
class SceneImpl {
    constructor(file, pageIndex) {
        ...
        this.root = buildSceneNode(pageRootNode(file, pageIndex), (0, math_1.identity)(), null, this.index);
    ...
    connectorHit(node, point) {
        ...
        for (let i = 0; i < pts.length - 1; i++) {
            if (distToSegment(point, pts[i], pts[i + 1]) <= tol)
                return true;
        }
        return false;
    }
    nodesUsingAsset(assetId) {
    ...
    connectorBounds(nodeId) {
        const sn = this.index.get(nodeId);
        if (!sn || sn.node.type !== "connector")
            return null;
        const conn = sn.node;
        if (!conn.start || !conn.end)
            return null;
    ...
    invalidateRegion(rect) {
        this.dirty = this.dirty ? (0, math_1.rectUnion)(this.dirty, rect) : { ...rect };
    }
    dirtyRegion() {
        return this.dirty;
    }
    ...
    hitTest(point, opts = {}) {
        ...
        const visit = (sn) => {
            ...
            if (sn === this.root)
                return null; // the implicit page root is not selectable
            ...
            if (node.type === "connector")
                return this.connectorHit(node, point) ? node : null;
            const inv = (0, math_1.invert)(mat(sn));
            if (!inv)
                return null;
            const local = (0, math_1.applyToPoint)(inv, point);
            return (0, hit_1.pointInLocalShape)(node, local, precise) ? node : null;
        };
        return visit(this.root);
    }
    queryViewport(rect) {
        if (!this.spatial) {
            const idx = new spatial_1.SpatialIndex();
            for (const sn of this.index.values()) {
                if (sn === this.root || (0, schema_1.isContainer)(sn.node))
                    continue; // selectable leaves only
    ...
    hitTestRect(rect, mode) {
        ...
        for (const sn of top) {
            ...
            const ok = mode === "contain"
                ? (0, math_1.rectContainsRect)(rect, sn.worldBounds)
                : (0, math_1.rectIntersects)(rect, sn.worldBounds);
            if (ok)
                out.push(sn.node);
        }
        return out;
...
function createScene(file, pageIndex = 0) {
