...
exports.buildClipFromPathData = buildClipFromPathData;
...
function parsePathCommands(d) {
    ...
    for (const g of groups) {
        ...
        let i = 0;
        const rx = (v) => (rel ? cur.x + v : v);
        const ry = (v) => (rel ? cur.y + v : v);
        const lineTo = (p) => { cmds.push({ op: "L", p }); cur = p; prevCtrl = null; };
        if (O === "M") {
            ...
            while (i + 1 < nums.length) {
                lineTo({ x: rx(nums[i]), y: ry(nums[i + 1]) });
                i += 2;
            }
        }
        else if (O === "H") {
            while (i < nums.length) {
                lineTo({ x: rel ? cur.x + nums[i] : nums[i], y: cur.y });
                i += 1;
            }
        }
        else if (O === "V") {
            while (i < nums.length) {
                lineTo({ x: cur.x, y: rel ? cur.y + nums[i] : nums[i] });
                i += 1;
            }
        }
        else if (O === "C") {
            ...
            while (i + 3 < nums.length) {
                const reflect = prevCtrl && (prevOp === "C" || prevOp === "S") ? { x: 2 * cur.x - prevCtrl.x, y: 2 * cur.y - prevCtrl.y } : { ...cur };
            ...
            while (i + 3 < nums.length) {
                const q = { x: rx(nums[i]), y: ry(nums[i + 1]) };
                const p = { x: rx(nums[i + 2]), y: ry(nums[i + 3]) };
                i += 4;
            ...
            while (i + 1 < nums.length) {
                const q = prevCtrl && (prevOp === "Q" || prevOp === "T") ? { x: 2 * cur.x - prevCtrl.x, y: 2 * cur.y - prevCtrl.y } : { ...cur };
                const p = { x: rx(nums[i]), y: ry(nums[i + 1]) };
                i += 2;
                cmds.push(quadToCubic(cur, q, p));
                cur = p;
 ...
 */
function buildClipFromPathData(ctx, d, w, h) {
    ...
    const tx = (p) => ({ x: (p.x - minX) * sx, y: (p.y - minY) * sy });
    let drew = false;
    for (const c of cmds) {
        if (c.op === "M") {
            const p = tx(c.p);
