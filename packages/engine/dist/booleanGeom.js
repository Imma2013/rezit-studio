...
function transformPath(vp, t) {
    const m = (0, math_1.fromTransform)(t);
    const pt = (p) => (0, math_1.applyToPoint)(m, p);
    return {
        ...
        subpaths: vp.subpaths.map((sp) => ({
            ...
            anchors: sp.anchors.map((a) => {
                const moved = pt(a);
                const out = { x: moved.x, y: moved.y, corner: a.corner };
                if (a.inHandle)
                    out.inHandle = pt(a.inHandle);
                if (a.outHandle)
                    out.outHandle = pt(a.outHandle);
                return out;
            }),
        })),
    };
 ...
 */
function booleanGeometry(node) {
    const operands = node.operands ?? [];
    if (operands.length < 2)
        return null;
    const paths = [];
    for (const operand of operands) {
        const parametric = (0, geometry_1.shapeNodeToParametric)(operand);
        if (!parametric)
            return null; // a `custom` operand carries pathData, not a parametric form
        paths.push(transformPath((0, geometry_1.shapeToPath)(parametric), operand.transform));
    }
    const combined = (0, geometry_1.booleanOp)(node.op, paths);
    if (!combined.subpaths.length)
        return null;
