...
exports.IDENTITY_PATCH = void 0;
exports.evalEasing = evalEasing;
exports.cubicBezierEase = cubicBezierEase;
exports.clipEase = clipEase;
exports.entrancePatch = entrancePatch;
exports.entranceProgress = entranceProgress;
exports.exitPatch = exitPatch;
exports.emphasisPatch = emphasisPatch;
exports.clipEnd = clipEnd;
exports.customPatch = customPatch;
exports.customTrackEnd = customTrackEnd;
exports.appliedOpacity = appliedOpacity;
exports.transitionProgress = transitionProgress;
exports.imageMotionPatch = imageMotionPatch;
exports.IDENTITY_PATCH = Object.freeze({
    dx: 0,
    dy: 0,
    scale: 1,
    rotate: 0,
 ...
 */
function clipProgress(clip, tMs) {
...
function entrancePatch(clip, tMs) {
...
function entranceProgress(clip, tMs) {
    const raw = clipProgress(clip, tMs);
 ...
 */
function emphasisPatch(clip, tMs) {
...
function customPatch(track, tMs) {
    ...
    if (t <= sorted[0].t)
        return keyframePatch(sorted[0]);
    const last = sorted[sorted.length - 1];
    if (t >= last.t)
        return keyframePatch(last);
    let i = 0;
    while (i < sorted.length - 1 && sorted[i + 1].t <= t)
        i++;
    const a = sorted[i];
    const b = sorted[i + 1];
 ...
 */
function appliedOpacity(baseOpacity, opacityMul) {
 ...
 */
function transitionProgress(tMs, durationMs) {
    if (durationMs <= 0)
        return 1;
 ...
 */
function imageMotionPatch(motion, tMs, periodMs = 12000) {
