...
exports.pointInLocalShape = pointInLocalShape;
function inBox(p, w, h) {
    return p.x >= 0 && p.x <= w && p.y >= 0 && p.y <= h;
}
/** Regular polygon / star vertices inscribed in the node box. */
function polygonPoints(w, h, sides, innerRatio) {
    const cx = w / 2;
    const cy = h / 2;
    const rx = w / 2;
    const ry = h / 2;
 ...
 */
function pointInLocalShape(node, p, precise) {
    ...
    switch (node.type) {
        case "shape": {
            ...
            if (shape === "polygon" || shape === "star") {
                const sides = node.sides ?? 5;
                const inner = shape === "star" ? (node.innerRadius ?? 0.5) : undefined;
        ...
        case "line": {
            ...
            const half = Math.max(2, (node.stroke?.width ?? 1) / 2);
            for (let i = 0; i < pts.length - 1; i++) {
                if (distToSegment(p, pts[i], pts[i + 1]) <= half)
                    return true;
            }
            return false;
        }
        case "ink": {
            ...
            // bounding box is not.
            const pts = node.points ?? [];
            const half = Math.max(3, (node.brush?.width ?? 1) / 2 + 2);
            if (pts.length === 1)
                return distToSegment(p, pts[0], pts[0]) <= half;
            for (let i = 0; i < pts.length - 1; i++) {
                if (distToSegment(p, pts[i], pts[i + 1]) <= half)
                    return true;
            }
