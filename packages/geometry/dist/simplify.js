...
exports.fitCubicBeziers = fitCubicBeziers;
function dist(a, b) {
...
function fitOneCubic(points, tHat0, tHat3, u) {
    const p0 = points[0];
...
function maxFitError(points, bez) {
    const STEPS = 48;
    const curve = [];
    for (let s = 0; s <= STEPS; s++)
        curve.push(cubicAt(bez, s / STEPS));
    let error = 0;
...
function fitRecursive(points, tHat0, tHat3, tolerance, depth) {
    if (points.length < 2)
        return [];
    if (points.length === 2) {
    ...
    if (error <= tolerance || depth > 24 || index <= 0 || index >= points.length - 1)
        return [bez];
