...
const qr_tables_1 = require("./qr-tables");
function dataCapacityCodewords(version, ec) {
    const entry = qr_tables_1.BLOCK_TABLE[version - 1][EC_LEVEL_INDEX[ec]];
    const g1d = entry[2];
    const b1 = entry[1];
    const b2 = entry[3];
    const g2d = entry[4];
...
function blankMatrix(size) {
    return Array.from({ length: size }, () => Array.from({ length: size }, () => ({ dark: false, reserved: false })));
}
function placeFinder(m, r, c) {
    for (let dr = -1; dr <= 7; dr++) {
        for (let dc = -1; dc <= 7; dc++) {
            const rr = r + dr;
            const cc = c + dc;
            if (rr < 0 || cc < 0 || rr >= m.length || cc >= m.length)
                continue;
...
function placeAlignment(m, version) {
    const positions = qr_tables_1.ALIGN_POS[version - 1];
...
function reserveFormatAndVersion(m, version) {
    ...
    for (let i = 0; i < 8; i++) {
        if (!m[8][size - 1 - i].reserved)
            m[8][size - 1 - i].reserved = true;
        if (!m[size - 1 - i][8].reserved)
...
function placeData(m, codewords) {
    ...
    const isData = Array.from({ length: size }, () => new Array(size).fill(false));
...
function placeFormat(m, reserved, ec, mask) {
    const size = m.length;
