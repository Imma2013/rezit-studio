...
exports.quantityTierMultiplier = quantityTierMultiplier;
exports.quote = quote;
 ...
 */
function quantityTierMultiplier(qty) {
    if (qty >= 100)
        return 0.7;
    if (qty >= 50)
        return 0.8;
...
function baseUnitCents(product, sizeId) {
    const table = product.basePriceCents;
    if (table && typeof table[sizeId] === "number")
        return table[sizeId];
 ...
 */
function quote(product, sizeId, substrateId, finishId, quantity, opts = {}) {
    if (!Number.isInteger(quantity) || quantity <= 0) {
        throw new Error("quantity must be a positive integer");
    }
    if (!(0, catalog_1.findSize)(product, sizeId)) {
        throw new Error(`size ${sizeId} not found on product ${product.id}`);
    }
    const substrate = product.substrates.find((s) => s.id === substrateId);
    if (!substrate)
        throw new Error(`substrate ${substrateId} not found on product ${product.id}`);
    let finishDelta = 0;
    if (finishId !== undefined) {
        const finish = product.finishes.find((f) => f.id === finishId);
        if (!finish)
            throw new Error(`finish ${finishId} not found on product ${product.id}`);
    ...
    const totalCents = Math.max(0, baseCents + optionsCents + shippingCents + taxesCents - subsidyCents);
    return {
