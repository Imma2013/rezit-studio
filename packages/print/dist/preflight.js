...
function buildExportRequest(design, product, pageIndex) {
...
function isOverprintRisk(rec) {
    ...
    for (const f of fills) {
        ...
        if (srgb && srgb.r <= 0.01 && srgb.g <= 0.01 && srgb.b <= 0.01) {
            return true;
        }
    }
    return false;
 ...
 */
function runPrintPreflight(design, product, sizeId, opts = {}) {
    ...
    const report = opts.exportReport ?? (0, export_1.preflight)(design, buildExportRequest(design, product, pageIndex));
    ...
    if (report.lowResImages.length === 0) {
        ...
        for (const img of report.lowResImages) {
            const level = img.ppi < product.requiredDpi * 0.75 ? "error" : "warn";
            checks.push({
                code: "dpi",
                level,
    ...
    if (product.colorSpace === "CMYK") {
        ...
        if (looksRgb) {
            checks.push({
                code: "color_space",
                level: "warn",
                message: "design is RGB but the product prints in CMYK; colors will be converted and may shift",
                overridable: true,
            });
        }
        else {
    ...
    const safeInsetPx = (0, geometry_1.mmToPx)(product.safeZoneMm, design.dpi);
 ...
 */
function evaluateGate(result, overrides = new Set()) {
    const blocking = [];
    const acknowledged = [];
