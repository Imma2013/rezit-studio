...
function strokeXml(stroke) {
    const st = stroke;
    if (!st || !st.width || st.width <= 0)
        return "";
...
function xfrmXml(x, y, w, h, rotationDeg, flipH, flipV) {
    let ox = x;
    let oy = y;
    if (rotationDeg) {
...
async function deckToPptx(file, opts = {}) {
    const pages = (file.pages ?? []);
    if (!pages.length)
        throw new Error("deckToPptx: design has no pages");
    const pageW = Math.max(1, Math.round(pages[0].width));
    const pageH = Math.max(1, Math.round(pages[0].height));
    const title = opts.title ?? file.title ?? "Presentation";
    const entries = [];
    const enc = new TextEncoder();
    const put = (name, xml) => entries.push({ name, data: enc.encode(xml) });
    const mediaSeq = { n: 0 };
    const contentTypeExts = new Set(["png"]); // png always declared (raster fallback)
    const slides = [];
    for (let pi = 0; pi < pages.length; pi++) {
        slides.push(await buildSlide(pages[pi], pi));
    }
    async function buildSlide(page, pageIndex) {
        const shapes = [];
        const rels = [];
        const media = [];
        let sid = 2; // shape ids start after the group root
        const addImageRel = (data, mime) => {
        ...
        const rasterize = async (node) => {
            if (!opts.rasterizeNode)
                return; // no fallback available: skip
            const r = await opts.rasterizeNode(pageIndex, node.id);
            if (!r || !r.png.length)
                return;
            const relId = addImageRel(r.png, "image/png");
            shapes.push(`<p:pic><p:nvPicPr><p:cNvPr id="${sid++}" name="${esc(nodeName(node))}"/><p:cNvPicPr/><p:nvPr/></p:nvPicPr>` +
        ...
        const walk = async (nodes, tx, ty, sx, sy) => {
            for (const raw of nodes) {
                ...
                if (node.type === "group" || node.type === "frame") {
                    ...
                    const clips = node.type === "frame" && !!node.clip && kids.length > 0;
                    ...
                    if (node.type === "frame") {
                        ...
                        if (fills?.length) {
                            shapes.push(`<p:sp><p:nvSpPr><p:cNvPr id="${sid++}" name="Frame"/><p:cNvSpPr/><p:nvPr/></p:nvSpPr>` +
                ...
                if (node.type === "text") {
                    const paras = (node.content ?? []).map(paragraphXml).join("");
                    shapes.push(`<p:sp><p:nvSpPr><p:cNvPr id="${sid++}" name="${esc(nodeName(node))}"/><p:cNvSpPr txBox="1"/><p:nvPr/></p:nvSpPr>` +
                ...
                if (node.type === "shape") {
                    ...
                    const fills = node.fills ?? [];
                    const imageFill = fills.some((f) => f?.type === "image");
                    if (!prst || imageFill) {
                        await rasterize(node); // custom geometry / image fill: keep fidelity
                        continue;
                    }
                    shapes.push(`<p:sp><p:nvSpPr><p:cNvPr id="${sid++}" name="${esc(nodeName(node))}"/><p:cNvSpPr/><p:nvPr/></p:nvSpPr>` +
                        `<p:spPr>${xfrmXml(x, y, w, h, rot, flipH, flipV)}<a:prstGeom prst="${prst}"><a:avLst/></a:prstGeom>${fillXml(fills[0])}${strokeXml(node.stroke)}</p:spPr>` +
                ...
                if (node.type === "image") {
                    ...
                    const per = (v) => Math.round(Math.max(0, Math.min(1, v)) * 100000);
                    if (crop && crop.width > 0 && crop.height > 0) {
                        ...
                        if (visW < 0.999 || visH < 0.999)
                            srcRect = `<a:srcRect l="${per(l)}" t="${per(tp)}" r="${per(l)}" b="${per(tp)}"/>`;
                    }
                    shapes.push(`<p:pic><p:nvPicPr><p:cNvPr id="${sid++}" name="${esc(nodeName(node))}"/><p:cNvPicPr/><p:nvPr/></p:nvPicPr>` +
                ...
                if (node.type === "line") {
                    ...
                    if (pts && pts.length === 2) {
                        ...
                        shapes.push(`<p:cxnSp><p:nvCxnSpPr><p:cNvPr id="${sid++}" name="Line"/><p:cNvCxnSpPr/><p:nvPr/></p:nvCxnSpPr>` +
                            `<p:spPr>${xfrmXml(lx, ly, Math.max(lw, 0.5), Math.max(lh, 0.5), 0, false, !down)}<a:prstGeom prst="line"><a:avLst/></a:prstGeom>${strokeXml(node.stroke) || `<a:ln w="${emu(2)}">${solidFillXml({ r: 0, g: 0, b: 0, a: 1 })}</a:ln>`}</p:spPr></p:cxnSp>`);
                        continue;
                    }
                    await rasterize(node);
                    continue;
        ...
        await walk((page.children ?? []), 0, 0, 1, 1);
        // Page background: a solid/gradient becomes the slide <p:bg>.
        const bgFill = page.background;
        ...
        const notes = typeof page.notes === "string" && page.notes.trim() ? page.notes.trim() : null;
    ...
    slides.forEach((s, i) => {
        ...
        const notesRel = s.notes
            ...
            : "";
        put(`ppt/slides/_rels/slide${n}.xml.rels`, `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">${layoutRel}${notesRel}${s.rels.join("")}</Relationships>`);
        for (const m of s.media)
            entries.push({ name: `ppt/media/${m.name}`, data: m.data });
        if (s.notes) {
            put(`ppt/notesSlides/notesSlide${n}.xml`, `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>` +
            ...
            put(`ppt/notesSlides/_rels/notesSlide${n}.xml.rels`, `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rIdSlide" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slide" Target="../slides/slide${n}.xml"/></Relationships>`);
        }
    });
    ...
    put("ppt/_rels/presentation.xml.rels", `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">` +
    ...
    put("ppt/slideMasters/_rels/slideMaster1.xml.rels", `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">` +
    ...
    put("ppt/slideLayouts/_rels/slideLayout1.xml.rels", `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">` +
    ...
    put("docProps/core.xml", `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:title>${esc(title)}</dc:title><dc:creator>HyCanvas</dc:creator></cp:coreProperties>`);
    put("docProps/app.xml", `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"><Application>HyCanvas</Application><Slides>${slides.length}</Slides></Properties>`);
    return (0, zipstore_1.zipStore)(entries);
}
