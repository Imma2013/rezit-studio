...
const geometry_1 = require("@hc/geometry");
function num(n) {
...
function paintOf(fill, ctx) {
    if (!fill)
        return { ref: "none", opacity: 1 };
    if (fill.type === "solid")
        return { ref: rgbOf(fill.color), opacity: fill.color.srgb.a };
    if (fill.type === "gradient") {
        const id = `grad-${++ctx.gradId}`;
        const stops = fill.stops
            .map((s) => `<stop offset="${num(s.position)}" stop-color="${rgbOf(s.color)}" stop-opacity="${num(s.color.srgb.a)}"/>`)
            .join("");
        if (fill.gradient === "radial") {
            const c = fill.center ?? { x: 0.5, y: 0.5 };
            const r = fill.radius ?? 0.5;
            ctx.defs.push(`<radialGradient id="${id}" cx="${num(c.x)}" cy="${num(c.y)}" r="${num(r)}">${stops}</radialGradient>`);
        }
        else {
            ...
            ctx.defs.push(`<linearGradient id="${id}" x1="${num(0.5 - dx)}" y1="${num(0.5 - dy)}" x2="${num(0.5 + dx)}" y2="${num(0.5 + dy)}">${stops}</linearGradient>`);
        }
...
function strokeAttrs(stroke, ctx) {
    if (!stroke)
        return "";
    ...
    const rec = stroke;
    if (rec.cap)
        out += ` stroke-linecap="${esc(String(rec.cap))}"`;
    if (rec.join)
        out += ` stroke-linejoin="${esc(String(rec.join))}"`;
    if (Array.isArray(rec.dash) && rec.dash.length)
        out += ` stroke-dasharray="${rec.dash.map(num).join(",")}"`;
    return out;
}
/** Build an SVG path `d` from a VectorPath (cubic beziers via anchor handles). */
function vectorPathToD(vp) {
    const parts = [];
    for (const sub of vp.subpaths)
        parts.push(subPathToD(sub));
    return parts.join(" ");
}
function subPathToD(sub) {
    const a = sub.anchors;
    if (a.length === 0)
        return "";
...
function matrixAttr(node) {
    const m = (0, engine_1.fromTransform)(node.transform);
    return `matrix(${num(m.a)},${num(m.b)},${num(m.c)},${num(m.d)},${num(m.e)},${num(m.f)})`;
}
function shapeBody(node, ctx) {
    const rec = node;
    const w = node.size.width;
    const h = node.size.height;
    const paint = fillAttrs(rec.fills, ctx) + strokeAttrs(rec.stroke, ctx);
    const kind = rec.shape;
    if (kind === "rect") {
    ...
    if (kind === "polygon" || kind === "triangle") {
        vp = (0, geometry_1.shapeToPath)({ kind: "polygon", width: w, height: h, sides: kind === "triangle" ? 3 : Number(rec.sides) || 3 });
    }
    else if (kind === "star") {
        vp = (0, geometry_1.shapeToPath)({ kind: "star", width: w, height: h, points: Number(rec.sides) || 5, innerRatio: Number(rec.innerRadius) || 0.5 });
    }
    if (vp)
        return `<path d="${vectorPathToD(vp)}"${paint}/>`;
    return "";
}
function pathContourD(segs, closed) {
...
function pathBody(node, ctx) {
    ...
    for (const c of rec.contours ?? []) {
        const cs = c.segments ?? [];
        if (cs.length >= 2)
            parts.push(pathContourD(cs, !!c.closed));
    }
    const rule = parts.length > 1 ? ` fill-rule="evenodd"` : "";
    const paint = fillAttrs(rec.fills, ctx) + strokeAttrs(rec.stroke, ctx);
    return `<path d="${parts.join(" ")}"${rule}${paint}/>`;
}
function lineBody(node, ctx) {
    const rec = node;
    const pts = rec.points ?? [];
    const points = pts.map((p) => `${num(Number(p.x))},${num(Number(p.y))}`).join(" ");
    return `<polyline points="${points}" fill="none"${strokeAttrs(rec.stroke, ctx)}/>`;
}
function textBody(node, ctx) {
    const rec = node;
    ...
    for (const para of paras) {
        ...
        for (const run of runs) {
            ...
            const paint = paintOf(style.fill, ctx);
            const fo = paint.opacity < 1 ? ` fill-opacity="${num(paint.opacity)}"` : "";
            tspans.push(`<tspan font-family="${family}" font-size="${num(size)}" fill="${paint.ref}"${fo}>${esc(String(run.text ?? ""))}</tspan>`);
        }
        if (lineHeight === 0)
            lineHeight = 16 * 1.2;
        y += lineHeight;
...
function emitNode(node, ctx) {
    ...
    switch (node.type) {
        ...
        case "frame":
        case "grid":
            body = (0, schema_1.childrenOf)(node).map((c) => emitNode(c, ctx)).join("");
            break;
        default:
...
function toSvg(file, pageIndex, opts = { keepEditable: true, embedRasters: true }) {
    const page = file.pages[pageIndex];
    if (!page)
        throw new Error(`page index ${pageIndex} out of range`);
    const ctx = { defs: [], gradId: 0, file, opts };
    const bg = page.background && page.background.type !== "pattern" && page.background.type !== "image"
    ...
    return (`<svg xmlns="http://www.w3.org/2000/svg" width="${num(page.width)}" height="${num(page.height)}" ` +
