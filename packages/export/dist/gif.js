...
function quantize(samples, maxColors) {
    ...
    while (boxes.length < maxColors) {
        ...
        if (target < 0)
            break; // every box has a single colour
...
function encodeGif(frames, opts) {
    ...
    if (width <= 0 || height <= 0)
        throw new Error("encodeGif: invalid dimensions");
    ...
    let bitsPerPixel = 2;
    while (1 << bitsPerPixel < totalColors)
        bitsPerPixel++;
    ...
    for (const frame of frames) {
        ...
        for (let p = 0; p < pxPerFrame; p++) {
            ...
            if (transparentIndex >= 0 && a < transparentAlpha) {
                ...
                indices[p] = nearestIndex(paletteColors, frame.rgba[p * 4], frame.rgba[p * 4 + 1], frame.rgba[p * 4 + 2], cache);
            }
        }
