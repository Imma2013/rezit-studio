...
exports.trim = trim;
exports.splitClip = splitClip;
exports.rippleDelete = rippleDelete;
...
function withClips(track, clips) {
    return { ...track, clips: (0, model_1.sortClips)(clips) };
}
function replaceClip(track, clipId, next, extra = []) {
    const clips = [];
 ...
 */
function trim(track, clipId, edge, deltaFrames, opts = {}) {
    ...
    const sourceEdge = reversed ? (edge === "in" ? "out" : "in") : edge;
    ...
    if (sourceEdge === "in") {
        ...
        if (newIn < 0)
            newIn = 0;
        if (newIn > clip.outFrame - 1)
            newIn = clip.outFrame - 1;
        next.inFrame = newIn;
    }
    else {
        let newOut = clip.outFrame + sourceDelta;
        if (newOut < clip.inFrame + 1)
            newOut = clip.inFrame + 1;
    ...
    if (edge === "in") {
        ...
        if (newStart < 0) {
            ...
            if (reversed) {
                next.outFrame = Math.min(opts.maxSourceFrames ?? Number.MAX_SAFE_INTEGER, next.outFrame + giveBack);
            }
            else {
 ...
 */
function splitClip(track, clipId, atFrame) {
    const clip = track.clips.find((c) => c.id === clipId);
    if (!clip)
        return track;
    const start = clip.startFrame;
    const end = (0, model_1.clipEndFrame)(clip);
    if (atFrame <= start || atFrame >= end)
        return track; // must be strictly inside
    ...
    const leftEdges = { transitionOut: undefined, fadeOutFrames: undefined };
    const rightEdges = { transitionIn: undefined, fadeInFrames: undefined };
    if (clip.title) {
    ...
    if (clip.speed < 0) {
        ...
        const left = { ...cloneClip(clip), inFrame: clampedSrc, outFrame: clip.outFrame, ...leftEdges };
        const right = {
            ...cloneClip(clip),
            id: (0, model_1.genId)("clip"),
            ...
            startFrame: start + (0, model_1.clipDurationFrames)(left),
            ...rightEdges,
        };
    ...
    const clampedSrc = Math.min(clip.outFrame - 1, Math.max(clip.inFrame + 1, splitSrc));
    const left = { ...cloneClip(clip), inFrame: clip.inFrame, outFrame: clampedSrc, ...leftEdges };
    const right = {
        ...cloneClip(clip),
        id: (0, model_1.genId)("clip"),
        inFrame: clampedSrc,
 ...
 */
function rippleDelete(track, clipId) {
    const clip = track.clips.find((c) => c.id === clipId);
    if (!clip)
        return track;
    const removedDuration = (0, model_1.clipDurationFrames)(clip);
...
function moveClip(track, clipId, toStartFrame) {
    const clip = track.clips.find((c) => c.id === clipId);
    if (!clip)
        return track;
