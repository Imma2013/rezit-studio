 ...
 * exceeded.
 */
function nestClipRefsValid(project, resolve) {
    const visiting = new Set();
    const walk = (proj, depth) => {
        if (depth > exports.MAX_NEST_DEPTH)
            return false;
        for (const seqId of directSequenceRefs(proj)) {
            if (visiting.has(seqId))
                return false; // cycle
 ...
 */
function wouldCreateCycle(project, sequenceId, resolve, selfId) {
    // Direct self-reference.
    if (selfId && sequenceId === selfId)
        return true;
    ...
    const reaches = (seqId, depth) => {
        if (depth > exports.MAX_NEST_DEPTH)
            return true; // treat over-deep as a reject
        if (selfId && seqId === selfId)
            return true;
        if (onPath.has(seqId))
            return true;
