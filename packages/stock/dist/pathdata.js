...
exports.pathUsesArcs = pathUsesArcs;
function tokenize(d) {
...
function parsePathData(d) {
    ...
    let pos = { x: 0, y: 0 };
    let start = { x: 0, y: 0 };
    let prevCubicCtrl = null; // reflected control for S
    let prevQuadCtrl = null; // reflected control for T
    let i = 0;
    let cmd = "";
    ...
    while (i < tokens.length) {
        ...
        switch (C) {
            ...
            case "S": {
                const c1 = prevCubicCtrl ? { x: 2 * pos.x - prevCubicCtrl.x, y: 2 * pos.y - prevCubicCtrl.y } : { ...pos };
                const c2 = { x: num() + (rel ? pos.x : 0), y: num() + (rel ? pos.y : 0) };
                const end = { x: num() + (rel ? pos.x : 0), y: num() + (rel ? pos.y : 0) };
                if (cur)
                    lastSeg().cOut = c1;
            ...
            case "T": {
                const q = prevQuadCtrl ? { x: 2 * pos.x - prevQuadCtrl.x, y: 2 * pos.y - prevQuadCtrl.y } : { ...pos };
                const end = { x: num() + (rel ? pos.x : 0), y: num() + (rel ? pos.y : 0) };
                const c1 = { x: pos.x + (2 / 3) * (q.x - pos.x), y: pos.y + (2 / 3) * (q.y - pos.y) };
                const c2 = { x: end.x + (2 / 3) * (q.x - end.x), y: end.y + (2 / 3) * (q.y - end.y) };
                if (cur)
