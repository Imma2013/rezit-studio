...
exports.encodeQrMatrix = encodeQrMatrix;
...
function encodeQrMatrix(value, ecLevel = "M") {
    ...
    if (!entry)
        throw new Error("encodeQrMatrix: value too long for QR versions 1-6");
    ...
    for (let right = size - 1; right >= 1; right -= 2) {
        if (right === 6)
            right = 5; // skip the vertical timing column entirely
...
function penalty(m, size) {
    ...
    const matches = (get, start, pat) => {
        for (let i = 0; i < pat.length; i++)
            if (get(start + i) !== pat[i])
                return false;
        return true;
    };
    for (let y = 0; y < size; y++) {
        for (let x = 0; x <= size - 11; x++) {
            if (matches((i) => m[y][i], x, pat1) || matches((i) => m[y][i], x, pat2))
                score += 40;
        }
    }
    for (let x = 0; x < size; x++) {
        for (let y = 0; y <= size - 11; y++) {
            if (matches((i) => m[i][x], y, pat1) || matches((i) => m[i][x], y, pat2))
                score += 40;
        }
    }
