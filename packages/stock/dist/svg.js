...
exports.parseGradients = parseGradients;
exports.svgToNodes = svgToNodes;
const schema_1 = require("@hc/schema");
const pathdata_1 = require("./pathdata");
function parseAttrs(s) {
    ...
    for (const m of s.matchAll(/([\w:-]{1,64})\s*=\s*"([^"]*)"/g))
...
const srgb = (r, g, b, a = 1) => ({ srgb: { r: clamp01(r), g: clamp01(g), b: clamp01(b), a: clamp01(a) } });
...
function parseColor(v) {
    ...
    if (s === "none" || s === "transparent")
        return "none";
    if (s === "currentcolor")
        return srgb(0, 0, 0, 1); // no inherited `color` in the flat path -> black
    if (s.startsWith("#")) {
        let h = s.slice(1);
        if (h.length === 3)
            h = h.split("").map((c) => c + c).join("");
        else if (h.length === 4)
            h = h.split("").map((c) => c + c).join(""); // #rgba -> #rrggbbaa
    ...
    if (s.startsWith("hsl")) {
        ...
        if (![h, sa, l].every(Number.isFinite))
            return null;
        let a = 1;
        if (parts[3] !== undefined)
            a = parts[3].endsWith("%") ? parseFloat(parts[3]) / 100 : parseFloat(parts[3]);
...
function withAlpha(c, mult) {
    if (mult >= 1)
        return c;
    return { ...c, srgb: { ...c.srgb, a: clamp01((c.srgb.a ?? 1) * mult) } };
}
/** Apply an opacity multiplier to a fill (solid or gradient stops). */
function fillWithOpacity(f, mult) {
    if (mult >= 1)
        return f;
    if (f.type === "solid")
        return { ...f, color: withAlpha(f.color, mult) };
    if (f.type === "gradient")
        return { ...f, stops: f.stops.map((s) => ({ ...s, color: withAlpha(s.color, mult) })) };
    return f;
}
...
function parseGradients(svg) {
    ...
    for (const m of svg.matchAll(/<(linear|radial)Gradient\b([^>]*?)(?:\/>|>([\s\S]*?)<\/\1Gradient>)/gi)) {
        ...
        if (!id)
            continue;
        raw[id] = { kind: m[1].toLowerCase(), attrs, stops: m[3] ? parseStops(m[3]) : [], href: (attrs["xlink:href"] || attrs.href)?.replace(/^#/, "") };
    }
    const out = {};
    const stopsOf = (g, depth = 0) => {
        if (g.stops.length || !g.href || depth > 8)
            return g.stops;
...
function paintToFill(value, gradients, mult) {
    if (!value)
        return null;
    ...
    if (url) {
        ...
        if (rest) {
            ...
            if (fc === "none")
                return "none";
            if (fc)
                return fillWithOpacity({ type: "solid", color: fc }, mult);
        }
    ...
    if (c === "none")
        return "none";
    if (c)
        return fillWithOpacity({ type: "solid", color: c }, mult);
    return null;
}
function opacityOf(attrs, key) {
    const v = parseFloat(attrs[key]);
    return Number.isFinite(v) ? clamp01(v) : 1;
}
function fillsFrom(attrs, fallback, gradients) {
...
function strokeFrom(attrs, gradients) {
...
function rootInherited(svg) {
    const open = /<svg\b([^>]*)>/i.exec(svg)?.[1] ?? "";
    const all = parseAttrs(open);
    const out = {};
    for (const k of INHERITED_PAINT)
        if (all[k] !== undefined)
            out[k] = all[k];
    return out;
}
function bboxOfSegments(segs) {
...
function pathNodeFromSubs(subs, fills, id, stroke, opacity) {
...
function anchorToAlign(v) {
    return v === "middle" ? "center" : v === "end" ? "right" : "left";
}
function svgToNodes(svg, idGen = (() => { let i = 0; return () => `svg-${++i}`; })(), opts = {}) {
    const nodes = [];
    const assets = [];
    ...
    const ff = (attrs, want = true) => fillsFrom(attrs, fallbackFill && want, gradients);
    const inherited = rootInherited(svg);
    const withInherited = (own) => ({ ...inherited, ...own });
    ...
    for (const m of svg.matchAll(re)) {
        if (m[3] !== undefined) {
            ...
            const color = fc && fc !== "none" && fc.type === "solid" ? fc.color : { srgb: { r: 0, g: 0, b: 0, a: 1 } };
            ...
            nodes.push((0, schema_1.createNode)("text", {
                id: idGen(),
        ...
        if (tag === "image") {
            ...
            nodes.push((0, schema_1.createNode)("image", {
                id: idGen(),
            ...
            if (/[aA]/.test(attrs.d ?? ""))
                approximated = true;
            const subs = (0, pathdata_1.parsePathData)(attrs.d ?? "");
            const fills = ff(attrs, !stroke);
            if (subs.length)
                nodes.push(pathNodeFromSubs(subs, fills, idGen(), stroke, opP));
        }
        else if (tag === "rect") {
            const r = num(attrs, "rx", num(attrs, "ry", 0));
            nodes.push((0, schema_1.createNode)("shape", {
                id: idGen(),
                shape: "rect",
                transform: { x: num(attrs, "x"), y: num(attrs, "y"), scaleX: 1, scaleY: 1, rotation: 0 },
            ...
            if (segs.length)
                nodes.push(pathNodeFromSubs([{ segments: segs, closed: tag === "polygon" }], fills, idGen(), stroke, opP));
        }
        else if (tag === "line") {
            const segs = [
