...
exports.colorMatches = colorMatches;
exports.filtersToQuery = filtersToQuery;
exports.colorfulness = colorfulness;
exports.stockMatches = stockMatches;
...
function colorMatches(asset, hex, maxDist = 60) {
...
function filtersToQuery(params) {
    const q = {};
    if (params.q)
        q.text = params.q;
    if (params.kind)
        q.kind = params.kind.split(",");
    ...
    if (params.style)
        q.style = params.style.split(",");
    if (params.license)
        q.license = params.license.split(",");
    ...
    if (min !== undefined || max !== undefined)
        q.durationMs = { min, max };
    if (params.collection)
        q.collectionId = params.collection;
    if (params.sort === "newest")
        q.sort = "newest";
    return q;
}
function textRelevance(asset, q) {
    const needle = q.toLowerCase();
    if (asset.title.toLowerCase() === needle)
        return 100;
    let score = 0;
    if (asset.title.toLowerCase().includes(needle))
        score += 50;
    for (const t of asset.tags) {
        if (t.toLowerCase() === needle)
            score += 20;
        else if (t.toLowerCase().includes(needle))
            score += 8;
    }
    return score;
}
...
function stockMatches(asset, query) {
    if (query.text && textRelevance(asset, query.text) === 0)
        return false;
    const kinds = asArray(query.kind);
    if (kinds.length && !kinds.includes(asset.kind))
        return false;
    if (query.orientation && asset.orientation !== query.orientation)
        return false;
    if (query.category && asset.category !== query.category)
        return false;
    if (query.color && !colorMatches(asset, query.color))
        return false;
    const styles = asArray(query.style);
    if (styles.length && !styles.some((s) => (asset.style ?? []).includes(s)))
        return false;
    const licenses = asArray(query.license);
    if (licenses.length && !licenses.includes(asset.license.type))
        return false;
    if (query.durationMs) {
        const d = asset.durationMs ?? 0;
        if (query.durationMs.min !== undefined && d < query.durationMs.min)
            return false;
        if (query.durationMs.max !== undefined && d > query.durationMs.max)
            return false;
    }
    if (query.collectionId && !asset.collectionIds.includes(query.collectionId))
        return false;
    return true;
}
...
function searchStock(assets, query = {}) {
    const matched = assets.filter((a) => stockMatches(a, query));
    if (query.sort === "newest")
        return matched; // caller supplies newest-first order
    if (!query.text) {
        return matched
