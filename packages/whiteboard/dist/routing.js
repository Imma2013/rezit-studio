...
exports.routeConnector = routeConnector;
/** Center of a box. */
 ...
 */
function anchorPoint(box, anchor, toward) {
 ...
 */
function resolveEndpoint(ep, boxes, otherRef) {
...
function endpointReference(ep, boxes) {
    if (ep.attach && boxes[ep.attach.nodeId])
        return boxCenter(boxes[ep.attach.nodeId]);
    if (ep.point)
        return { x: ep.point.x, y: ep.point.y };
    return undefined;
}
 ...
 */
function routeConnector(conn, boxes) {
