...
exports.normalizeDiagramSpec = normalizeDiagramSpec;
exports.diagramToMermaid = diagramToMermaid;
exports.mermaidToDiagram = mermaidToDiagram;
const MAX_NODES = 60;
const MAX_EDGES = 120;
const MAX_LABEL = 120;
/**
 ...
 */
function normalizeDiagramSpec(raw) {
...
function diagramToMermaid(spec) {
 ...
 */
function mermaidToDiagram(source) {
    ...
    for (const line of lines.slice(1)) {
        if (/^(subgraph|end$|style|classDef|class |linkStyle|click )/i.test(line))
            continue;
        ...
        if (em) {
            ...
            note(bId, bLabel);
            if (aId !== bId)
                edges.push({ from: aId, to: bId, ...(edgeLabel?.trim() ? { label: edgeLabel.trim() } : {}) });
            continue;
        }
