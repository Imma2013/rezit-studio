...
exports.whiteboardToDeck = whiteboardToDeck;
const schema_1 = require("@hc/schema");
const region_1 = require("./region");
function isFrame(node) {
    return node.type === "frame";
}
function topLevelFrames(design) {
    const frames = [];
 ...
 */
function fitPageToSlide(page, w, h) {
 ...
 */
function spatialSectionSlide(design, frame, w, h) {
    ...
    const children = inside.map((n) => {
        ...
        regenIds(c);
        if (c.transform)
            c.transform = { ...c.transform, x: c.transform.x - fb.x, y: c.transform.y - fb.y };
        return c;
    });
    const page = {
        id: (0, schema_1.newId)(),
...
function whiteboardToDeck(design, opts = {}) {
    const w = opts.slideWidth ?? 1920;
    const h = opts.slideHeight ?? 1080;
    const deck = (0, schema_1.createBlankDesign)({ title: opts.title ?? `${design.title} (deck)`, width: w, height: h });
    ...
    if (frames.length > 0) {
        for (const frame of frames) {
            ...
            if ((frame.children?.length ?? 0) > 0) {
                const extracted = (0, region_1.extractRegion)(design, { frameId: frame.id });
                for (const page of extracted.pages)
                    pages.push(fitPageToSlide(page, w, h));
            }
            else {
        ...
        const extracted = (0, region_1.extractRegion)(design, { rect: unionBounds(all) });
        for (const page of extracted.pages)
            pages.push(fitPageToSlide(page, w, h));
    }
