...
exports.fitStickyFontScale = fitStickyFontScale;
 ...
 */
function fitStickyFontScale(text, width, height, opts = {}) {
    ...
    if (chars === 0)
        return clamp(maxScale, minScale, maxScale);
    ...
    if (fits(hi))
        return clamp(maxScale, minScale, maxScale);
    if (!fits(lo))
        return clamp(minScale, minScale, maxScale);
    for (let i = 0; i < 40; i++) {
        const mid = (lo + hi) / 2;
        if (fits(mid))
            lo = mid;
