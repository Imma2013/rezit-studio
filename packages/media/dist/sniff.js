...
function matches(bytes, sig, offset = 0) {
    for (let i = 0; i < sig.length; i++) {
        if (at(bytes, offset + i) !== sig[i])
            return false;
    }
    return true;
}
...
function leadingText(bytes, max = 512) {
    let s = "";
    const n = Math.min(max, bytes.length ?? 0);
    for (let i = 0; i < n; i++) {
        const c = at(bytes, i);
        if (c < 0)
            break;
...
function sniffType(bytes) {
    ...
    if (matches(bytes, [0x47, 0x49, 0x46, 0x38]))
        return { mime: "image/gif", kind: "gif" }; // GIF8
    if (matches(bytes, [0x42, 0x4d]))
        return { mime: "image/bmp", kind: "image" }; // BM
    ...
    if (asciiAt(bytes, 4, "ftyp")) {
        ...
        if (brand === "heic" || brand === "heif" || brand === "mif1" || brand === "hevc")
    ...
    if (matches(bytes, [0x49, 0x44, 0x33]) || matches(bytes, [0xff, 0xfb]) || matches(bytes, [0xff, 0xf3]))
        return { mime: "audio/mpeg", kind: "audio" };
    // Fonts
    if (matches(bytes, [0x4f, 0x54, 0x54, 0x4f]))
        return { mime: "font/otf", kind: "font" }; // OTTO
    if (matches(bytes, [0x00, 0x01, 0x00, 0x00]) || asciiAt(bytes, 0, "true"))
        return { mime: "font/ttf", kind: "font" };
    if (asciiAt(bytes, 0, "wOFF"))
        return { mime: "font/woff", kind: "font" };
    if (asciiAt(bytes, 0, "wOF2"))
        return { mime: "font/woff2", kind: "font" };
    // 3D
    if (asciiAt(bytes, 0, "glTF"))
        return { mime: "model/gltf-binary", kind: "model3d" };
    // PSD
    if (asciiAt(bytes, 0, "8BPS"))
        return { mime: "image/vnd.adobe.photoshop", kind: "source" };
 ...
 */
function acceptUpload(bytes, _declaredExt) {
