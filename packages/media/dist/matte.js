...
exports.growMatte = growMatte;
exports.shrinkMatte = shrinkMatte;
exports.featherMatte = featherMatte;
exports.refineMatte = refineMatte;
exports.brushMatte = brushMatte;
exports.applyMatteToRGBA = applyMatteToRGBA;
    ...
    for (let y = 0; y < h; y++) {
        ...
        for (let x = 0; x < w; x++) {
            ...
            for (let dx = -radius; dx <= radius; dx++)
                v = pick(v, src[row + Math.min(w - 1, Math.max(0, x + dx))]);
            tmp[row + x] = v;
        }
    }
    for (let x = 0; x < w; x++) {
        for (let y = 0; y < h; y++) {
            ...
            for (let dy = -radius; dy <= radius; dy++)
                v = pick(v, tmp[Math.min(h - 1, Math.max(0, y + dy)) * w + x]);
            out[y * w + x] = v;
        }
    }
    return out;
...
function growMatte(alpha, w, h, radius) {
    return morph(alpha, w, h, radius, true);
}
/** Shrink (erode) the matte by `radius` pixels. */
function shrinkMatte(alpha, w, h, radius) {
    return morph(alpha, w, h, radius, false);
}
/** Feather the matte edge with a blur of `radius` pixels. */
function featherMatte(alpha, w, h, radius) {
    return boxBlur(alpha, w, h, radius);
}
/** Classic "refine edges": optional grow, then shrink (choke), then feather. */
function refineMatte(alpha, w, h, opts = {}) {
    let a = alpha;
    if (opts.grow && opts.grow > 0)
        a = growMatte(a, w, h, Math.round(opts.grow));
    if (opts.shrink && opts.shrink > 0)
        a = shrinkMatte(a, w, h, Math.round(opts.shrink));
    if (opts.feather && opts.feather > 0)
        a = featherMatte(a, w, h, Math.round(opts.feather));
...
function brushMatte(alpha, w, h, stamp) {
...
function applyMatteToRGBA(rgba, alpha) {
    const n = Math.min(rgba.length / 4, alpha.length);
    for (let i = 0; i < n; i++)
        rgba[i * 4 + 3] = Math.round((rgba[i * 4 + 3] * alpha[i]) / 255);
    return rgba;
}
