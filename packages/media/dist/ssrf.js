...
exports.validateImportUrl = validateImportUrl;
...
function parseUrl(url) {
    ...
    if (at >= 0)
        authority = authority.slice(at + 1); // drop userinfo
    let host;
    let port;
    if (authority.startsWith("[")) {
        ...
        if (rest.startsWith(":"))
            port = Number(rest.slice(1)) || undefined;
    }
    else {
...
function isPrivateIp(host) {
    ...
    if (host.includes(":")) {
        ...
        if (lead.startsWith("fe8") || lead.startsWith("fe9") || lead.startsWith("fea") || lead.startsWith("feb"))
            return true; // fe80::/10 link-local
        if (lead.startsWith("fc") || lead.startsWith("fd"))
...
function validateImportUrl(url, opts = {}) {
    const parsed = parseUrl(url);
    if (!parsed)
        return { ok: false, reason: "invalid or non-absolute URL" };
    const schemes = opts.allowedSchemes ?? ["http", "https"];
    if (!schemes.includes(parsed.scheme))
        return { ok: false, reason: `scheme not allowed: ${parsed.scheme}`, parsed };
    if (!parsed.host)
        return { ok: false, reason: "empty host", parsed };
    ...
    if (isPrivateIp(parsed.host))
        return { ok: false, reason: "private or reserved IP blocked", parsed };
    if (opts.allowlist && opts.allowlist.length > 0) {
        ...
        if (!ok)
            return { ok: false, reason: "host not in allowlist", parsed };
    }
