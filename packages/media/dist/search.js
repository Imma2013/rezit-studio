...
function textMatches(asset, q) {
    const needle = q.toLowerCase();
    if (asset.name.toLowerCase().includes(needle))
        return true;
    if (asset.meta.altText?.toLowerCase().includes(needle))
        return true;
    return asset.tags.some((t) => t.value.toLowerCase().includes(needle));
}
/** Whether a single asset satisfies a query. */
function matchAsset(asset, query) {
    ...
    if (query.text && !textMatches(asset, query.text))
        return false;
    if (query.kind) {
    ...
    if (query.color && !colorMatches(asset, query.color))
        return false;
    if (query.orientation && orientationOf(asset) !== query.orientation)
        return false;
    if (query.minWidth !== undefined && (asset.meta.width ?? 0) < query.minWidth)
        return false;
    if (query.minHeight !== undefined && (asset.meta.height ?? 0) < query.minHeight)
        return false;
    if (query.durationMs) {
    ...
    if (query.createdAfter && asset.createdAt < query.createdAfter)
        return false;
    if (query.createdBefore && asset.createdAt > query.createdBefore)
        return false;
    return true;
}
