...
exports.alignDeltas = alignDeltas;
exports.distributeDeltas = distributeDeltas;
exports.tidyUpDeltas = tidyUpDeltas;
exports.order = order;
function alignDeltas(items, edge, target) {
    const out = new Map();
...
function distributeDeltas(items, axis, by) {
    const out = new Map();
    if (items.length < 3)
        return out;
    const pos = (b) => (axis === "h" ? b.x : b.y);
 ...
 */
function tidyUpDeltas(items, defaultGap = 16) {
    const out = new Map();
    if (items.length === 0)
        return out;
...
function order(children, ids, op) {
