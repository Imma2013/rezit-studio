...
function inferAxis(pos, extent, pageExtent) {
...
function mapAxis(plan, extent, targetExtent, factor) {
        ...
        for (const para of content) {
            ...
            for (const run of para.runs ?? []) {
                if (run.style?.fontSize != null)
                    run.style.fontSize = clampFont(run.style.fontSize);
                if (run.overrides?.fontSize != null)
                    run.overrides.fontSize = clampFont(run.overrides.fontSize);
            }
        }
    }
 ...
 */
function resizePage(page, target) {
    ...
    for (const node of out.children) {
        ...
        if (node.type === "line") {
            ...
            if (rec.points)
                rec.points = rec.points.map((p) => ({ ...p, x: p.x * kx, y: p.y * ky }));
        }
