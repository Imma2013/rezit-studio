...
exports.toolCatalog = toolCatalog;
exports.parseAssistantReply = parseAssistantReply;
exports.planMutates = planMutates;
exports.summarizeDesign = summarizeDesign;
exports.assistantSystemPrompt = assistantSystemPrompt;
...
function validateStep(action, rawArgs, byName) {
...
function parseAssistantReply(parsed, catalog) {
    ...
    if (clarify)
        return { reply: reply || clarify, clarify, plan: [] };
...
function planMutates(plan, catalog) {
    const byName = new Map(catalog.map((t) => [t.name, t]));
...
function summarizeDesign(doc, activePage, selectionCount) {
    const lines = [];
    if (doc.title)
...
function assistantSystemPrompt(catalog, designSummary) {
    const tools = catalog
        .map((t) => {
