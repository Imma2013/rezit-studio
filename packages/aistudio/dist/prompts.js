...
function outlineSystemPrompt(designType, brandClause, pageCount) {
...
function outlineUserPrompt(prompt, designType) {
...
function groundImagePrompt(prompt, ctx) {
    const parts = [prompt.trim()];
    if (ctx.style)
        parts.push(`Style: ${ctx.style}.`);
    if (ctx.palette && ctx.palette.length)
        parts.push(`Use a color palette consistent with: ${ctx.palette.slice(0, 6).join(", ")}.`);
    if (ctx.aspect)
        parts.push(`Composition suited to a ${ctx.aspect} frame.`);
