...
function deckThemes(opts) {
    const count = Math.max(1, Math.min(8, Math.round(opts.count)));
    const brand = (opts.brandPalette ?? [])
        .map(color_1.fromHex)
        .filter((c) => Boolean(c));
    const bases = brand.length
        ? brand
        : DEFAULT_HUES.map(color_1.fromHex).filter((c) => Boolean(c));
    ...
    for (let i = 0; i < count; i++) {
        ...
        let background;
        if (treatment === 0) {
            background = { kind: "solid", color: (0, color_1.toHex)(base) };
        }
        else if (treatment === 1) {
            background = {
                kind: "gradient",
                color: (0, color_1.toHex)(base),
                color2: (0, color_1.toHex)(darken(rotateHue(base, 18), 0.35)),
                angle: 135,
            };
        }
        else {
            background = {
                kind: "gradient",
                color: (0, color_1.toHex)(darken(base, 0.2)),
                color2: (0, color_1.toHex)(rotateHue(base, -24)),
                angle: 200,
            };
        }
