...
function resolveBackground(spec) {
    const start = (0, color_1.fromHex)(spec.background.color ?? "#ffffff") ?? WHITE;
    if (spec.background.kind === "gradient" && spec.background.color2) {
        const end = (0, color_1.fromHex)(spec.background.color2) ?? start;
        return {
            fill: {
                type: "gradient",
                gradient: "linear",
...
function minContrast(fg, refs) {
    let m = Infinity;
    for (const ref of refs)
        m = Math.min(m, (0, color_1.contrastRatio)(fg, ref));
    return m === Infinity ? 0 : m;
}
/** The reference `fg` reads worst against (used to nudge toward AA). */
function worstRef(fg, refs) {
    let worst = refs[0];
    let worstC = (0, color_1.contrastRatio)(fg, refs[0]);
    for (const ref of refs) {
        const c = (0, color_1.contrastRatio)(fg, ref);
        if (c < worstC) {
            worst = ref;
            worstC = c;
        }
...
function textColorFor(block, refs) {
    const hinted = block.color ? (0, color_1.fromHex)(block.color) : null;
    if (hinted && minContrast(hinted, refs) >= 4.5)
        return hinted;
...
function estimateLines(text, fontSize, boxWidth) {
...
function layoutDesign(spec, size) {
    ...
    spec.blocks.forEach((block, i) => {
        ...
        if (i < spec.blocks.length - 1)
            stackH += Math.round(shortSide * GAP_AFTER[spec.blocks[i].role]);
    });
    ...
    const gapPx = (role) => Math.round(shortSide * GAP_AFTER[role] * fit);
    ...
    const align = layout === "centered" ? "center" : rtl ? "right" : "left";
    const colLeft = layout === "centered"
    ...
    placed.forEach((p, i) => {
        ...
        if (p.block.role === "accent") {
            ...
            const barColor = p.block.color ? (0, color_1.fromHex)(p.block.color) ?? readableOn(bgRefs) : readableOn(bgRefs);
        ...
        if (i < placed.length - 1)
            cursorY += gapPx(placed[i].block.role);
    });
